# Plans.md FROZEN LEGACY ARCHIVE (2026-07-31)

> direct user instruction 2026-07-31「Plans.md内で完了済みタスクを削除して」により、
> Plans.md の FROZEN LEGACY INVENTORY(旧637〜2495行)を本ファイルへ全量退避した。
> 退避時点の Plans.md は HEAD 9d8dbc0 と一致しない未コミット provenance を含むため、
> 本ファイルが non-HEAD evidence の保全先である(旧 §10 の保全条件を充足)。
> 内容は NONCLAIMABLE の凍結履歴であり、claim・assignment・completion 判断に使用しない。

## FROZEN LEGACY INVENTORY — NONCLAIMABLE

Everything below this boundary is pre-existing dirty historical provenance. It
is not byte-identical to HEAD, not an active plan, and must not be used for
claim, assignment, completion, or priority. Do not append to it; active
dispositions above and Git history are the current records.
...
1. WP-4158はmixed-path `f4d0f8f`をS1 evidenceとして無効化し、同commitをrecovery baseとするexact4 recovery PLANをfresh reviewする。他6 pathを戻さず、変更せず、WP-4158 approval対象にも含めない。
...
- [~] WP-4158 map JP Core terminology rights provenance(MEDIUM read-only legal evidence) — S1_RECOVERY_PLAN_REVIEW / S0_REMOTE_CI_PASS / ORIGINAL_S1_GATES_SUPERSEDED
  ...
  - slice/ownership: root sole maintainer。S0 HEAD/base=`9a3e715f49532ea2b57bda8ec715b0f6c06435c0`でWP-4157 terminal projection + closure planのみ。Original S1はremote/CI-terminal S0 child + exact4を要求したが、`f4d0f8f71a62c3e55a57a79ad57092b10620cc70`（parent `68470672bd0f5efff6cc06f42cfb374dc59dc0f7`）がexact4に加えて`.omo/run-continuation/ses_09a245a19ffeBWzQa3pzsfAF7A.json`, `apps/api/package.json`, `apps/web/package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `scripts/check-scripts.mjs`を含む10-path mixed commitとしてmainへpush済みのため、original S1 landing/evidenceとしてINVALID/non-evidenceとする。履歴を書き換えず他6 pathをrevert/edit/approveせず、recovery baseをexact `f4d0f8f…`へ固定し、recovery commit自身のdiffだけをexact4とする。6 pathのmode/blob OIDはbaseとcandidateで完全一致を必須化する。Recovery後のS2はfresh six-gate + exact-head CI済み`LANDED_S1_RECOVERY_COMMIT`だけをbaseにでき、original `S1 parent=S0`を再主張しない。package/lock/runtime/code/CI/toolchain unchanged claimはrecovery commitのbase-relative diffだけに限定し、`f4d0f8f`の非exact4変更の妥当性・approvalを意味しない。persistent/repository artifact retention・展開、採用・公開・外部送信は禁止。
  ...
  - commit_push_oracles: AC9C external packetは40-hex `EXPECTED_BASE_COMMIT`, `EXPECTED_CANDIDATE_TREE`とexact `EXPECTED_MESSAGE`だけを入力とし、AC9L/AC9P external packetはcommit後にそれらへ40-hex `EXPECTED_COMMIT`を追加する。各phaseで必要な入力の欠落・型不正・command nonzero・schema/token不一致はfail-closed。exact path setは再帰的にexact4=`Plans.md,State.md,ops/refactor/EVIDENCE.md,ops/refactor/STATE.md`のみ（rename/copy/delete/submoduleを含むname-status driftもfail）。AC9C COMMIT readyは`HEAD=$EXPECTED_BASE_COMMIT`、tracked worktree diff name set=exact4、rootがreal indexへexact4だけを明示stage、cached name set=exact4、`git write-tree=$EXPECTED_CANDIDATE_TREE`、`git diff --cached --check` exit0、unstaged tracked diff empty、messageがsliceごとのexact値（S0 `WP-4158: reconcile terminology rights closure plan`; original S1 `WP-4158: verify terminology rights provenance`; S1 recovery r0001 `WP-4158: repair terminology rights S1 r0001`; S2 `WP-4158: record terminology rights landing`）をassertし、exact token=`WP4158_COMMIT_READY base=$EXPECTED_BASE_COMMIT tree=$EXPECTED_CANDIDATE_TREE paths=4 message=$EXPECTED_MESSAGE`。S1 recoveryではさらにbase=`f4d0f8f…`と、非exact4 6 pathのmode/blob OIDがbaseとcandidateで同一であることをassertする。fresh exactly5 COMMIT reviewersが同じartifactを5/5 PASS後にrootだけがcommitする。
  ...
  - ac9_local_commit_common_and_original_push: AC9L local prepush content oracleはoriginal/recovery共通で、commit後に`HEAD=$EXPECTED_COMMIT`、exactly one parent=`EXPECTED_BASE_COMMIT`、`git show -s --format=%T=$EXPECTED_CANDIDATE_TREE`、subject=`EXPECTED_MESSAGE`、parent→commit recursive name-status path set=exact4、tracked worktree empty、real index emptyをassertしtoken=`WP4158_LOCAL_COMMIT_PASS commit=$EXPECTED_COMMIT parent=$EXPECTED_BASE_COMMIT tree=$EXPECTED_CANDIDATE_TREE paths=4`。その後のhistorical branch/PR PUSH oracleだけはalready-terminal S0とoriginal historical S1/S2限定で、S1 recovery/S2 recoveryへ適用禁止。Recovery PUSHはsame fiveがcommon AC9Lを全員PASS後、下記`s1_recovery_landing_oracle`だけを使用する。
  ...
  - ac9p_remote_ci_original_only: このPR/fixed-main terminal oracleはalready-terminal S0とoriginal historical S1/S2だけに適用し、S1 recovery/S2 recoveryへ適用禁止。Historical AC9Pはworkflow `309812329` / event=`pull_request` / PR1と固定main guardを確認済み。Recoveryは下記`s1_recovery_landing_oracle`の`push`/`main` oracleだけを使用する。No tracked record claims its own future AC9 facts。
  ...
  - s1_recovery_plan_gate_oracle: original S1/S2 tokens above are historical and MUST NOT authorize recovery。Recovery PLAN external packet requires exact `EXPECTED_BASE_COMMIT=f4d0f8f71a62c3e55a57a79ad57092b10620cc70`、40-hex disposable `EXPECTED_CANDIDATE_TREE`、recursive exact4 path set、real index empty、candidate marker extraction SHA=`e096d18ce95dbd0f1167e12c918747393f23ceafdb48cc760e462d2d2c0bf151` / 2610 LF lines、non-ledger six mode/blob parity。Reviewers do not execute that known-racy source。Exact PLAN token=`WP4158_S1_RECOVERY_PLAN_READY base=$EXPECTED_BASE_COMMIT tree=$EXPECTED_CANDIDATE_TREE paths=4 source=e096d18ce95dbd0f1167e12c918747393f23ceafdb48cc760e462d2d2c0bf151 python_system=/usr/bin/python3@3.9.6 sys_system=/Library/Developer/CommandLineTools/usr/bin/python3 runtime_system=/Library/Developer/CommandLineTools/Library/Frameworks/Python3.framework/Versions/3.9/bin/python3.9 python_homebrew=/opt/homebrew/bin/python3@3.14.6 sys_homebrew=/opt/homebrew/opt/python@3.14/bin/python3.14 runtime_homebrew=/opt/homebrew/Cellar/python@3.14/3.14.6/Frameworks/Python.framework/Versions/3.14/bin/python3.14 signal_protected=40 signal_boundary=24 signal_combinations=54 mutations=11 remote=https://github.com/yusuketakuma/yrese.git remote_base=$EXPECTED_BASE_COMMIT remote_non_target=4f3185217ec6f2309bab38a8fdbf25112b8f141204b139449686f993039899f2 cas=exact-ff-lease human_gate=required verifier=planned`。Missing/mismatched input、old `WP4158_S1_PLAN_READY` reuse、future implementation execution、prototype result reuseはFAIL。Fresh exactly5全員が同じpacket/tokenをPASSして初めてRECOVERY IMPLEMENTATIONへ進む。
  - s1_recovery_interpreter_oracle: recovery executionはPATH lookupの`python3`を禁止し、absolute invocation paths `/usr/bin/python3`と`/opt/homebrew/bin/python3`を別processで使用する。Preflight childはversionに加えて`sys.executable`と`os.path.realpath(sys.executable)`をmachine-readable separate fieldsでemitし、systemはexact `/Library/Developer/CommandLineTools/usr/bin/python3` / `/Library/Developer/CommandLineTools/Library/Frameworks/Python3.framework/Versions/3.9/bin/python3.9`、Homebrewはexact `/opt/homebrew/opt/python@3.14/bin/python3.14` / `/opt/homebrew/Cellar/python@3.14/3.14.6/Frameworks/Python.framework/Versions/3.14/bin/python3.14`。Version stdoutはexact `Python 3.9.6\n` / `Python 3.14.6\n`、stderr empty、exit0。invocation path、`sys.executable`、resolved runtime identity、version、出力driftはSTOPしてPLAN改版へ戻す。Compatibility helperはnoteがstringであることを確認し、callable `add_note`があれば呼び、なければmissing `__notes__`をexact one-item listで作成、existing listへappend、non-listは`TypeError("Cannot add note: __notes__ is not a list")`とする。全secondary failure text/orderとprimary identity/type/message/original traceback tail identityを両interpreterで同じcase oracleによりassertする。各interpreterが独立にexact token `WP4158_RECOVERY_SELFTEST_PASS python=<3.9.6|3.14.6> positives=2 negatives=95 cleanup=10 signal_protected=40 signal_boundary=24 signal_combinations=54 mutations=11 residue=0`を一行だけemitし、片系PASSやversion/token混同はFAIL。
  - s1_recovery_signal_oracle: logical handoff cutoffは**second drainが取得したfinal pending snapshotの完了時点**。そのsnapshotまでに観測されたsignalだけをprotected deferred outcomeへ含め、snapshot完了後はmask中でもpost-handoffとしてrestored actual dispositionへ渡す。この不可避境界をraw unmask時点と誤称しない。Protected exact40 child names are `during_operation_pending|after_operation_before_drain1|after_drain1_before_restore|during_restore_masked|after_restore_before_drain2` × `success|failure` × `SIGINT|SIGTERM|SIGHUP|SIGALRM`。Success + SIGINT/TERM/HUPはexact `InterruptedError(signum)` type/args、success + SIGALRMはexact `VerificationError("TRANSPORT_LIMIT")`。Failureはsignal種別を問わずsame synthetic primary object/type/message=`synthetic transition primary`、reraised chain内のoriginal traceback tail object identity、exact expected chain shape、`__notes__ == ()`を要求する。全caseはpending empty、actual mask/handler/timer、root/canary/FD residue0、stdout exact `WP4158_SIGNAL_PROTECTED_PASS phase=<phase> outcome=<outcome> signal=<signal> residue=0\n`、stderr empty、exit0。Post-handoff exact24 names are `after_second_drain_before_deactivate|after_deactivate_before_outcome|after_outcome_before_unmask` × `success|failure` × same4 signals。Child sets `sys.tracebacklimit=0`; SIGINT requires returncode=-2、stdout empty、stderr exact `KeyboardInterrupt\n`; SIGTERM/HUP/ALRM require returncode=-15/-1/-14 and stdout/stderr empty。Post-handoff child cleanup is nonclaimed; the isolated parent verifies then removes only child-owned test residue and requires canary/repository/parent residue0。Static/controlled exact11 mutants are drain1 bypass、restore bypass、drain2 bypass、deactivate-before-drain2、final-outcome-read-before-drain2、unmask-before-drain2、unmask-before-final-outcome、allocation-call bypass、success-tail-call bypass、finalizer-call bypass、terminal-LF removal。First7 must fail only `LIVE_SIGNAL_TRANSITION_BINDING_FAILED`; last4 must fail only `LIVE_TRANSITION_BINDING_FAILED`。Existence/count-only guards、shared case names、unexpected exit/stdout/stderr/tokenはFAIL。
  - s1_recovery_multi_signal_oracle: protected pre-cutoff signals use deterministic state independent of Python set iteration and arrival/drain order。Any SIGALRM sets sticky timeout and timeout always wins a success outcome。Non-ALRM sticky selection uses exact priority `SIGINT > SIGTERM > SIGHUP`; repeated delivery is idempotent。Primary failure always wins all deferred signals/timeouts and keeps the same object/type/message/traceback-tail/empty notes, while state still records exact sticky timeout and selected non-ALRM priority for inspection。Uncounted-from-exact40 but token-bound exact54 controlled guards are: all 11 size≥2 subsets of the four signals injected in one `after_restore_before_drain2` snapshot × success/failure =22; every ordered distinct pair split across drain1/drain2 (12) × success/failure =24; same signal repeated across both drains (4) × success/failure =8。Each guard permutes input enumeration at least once and requires identical state/outcome。Post-handoff simultaneous-signal ordering follows OS actual disposition and is explicitly nondeterministic/nonclaimed。Missing combination、order-sensitive result、wrong timeout/priority/primary precedence fails only `LIVE_SIGNAL_COMBINATION_GUARD_FAILED`。
  - s1_recovery_push_runtime: historical branch/PR/main guards are original-only。Tool manifest V1 uses ordered `name NUL value LF` records and SHA `fcb6691cfaad9090e06ffabd15e24679ab2d36f84994e18d835a46770f08603f`: `/bin/bash` SHA `fde343ee184953c1fa1185abddeaa8be61c6acbebae4eb54db5d6b55b09a5755` / exact version line `GNU bash, version 3.2.57(1)-release (arm64-apple-darwin25)`; `/usr/bin/env` SHA `6e506aec3c0cff703ac1e66cedc6f1945354ad41339a38db4425c7c88227128f`; `/usr/bin/git` SHA `179301dcb41ea78accc3fa0048a7e6f6710d891945a751a34addd622020c1818` / `git version 2.50.1 (Apple Git-155)` / exec-path `/Library/Developer/CommandLineTools/usr/libexec/git-core`; `git-remote-https` SHA `badb3dc6666bdffdc3c6eaf44346eb390bb43d69b1608da4d1f971bbdb2d2998`; `/bin/sh` SHA `ad5c194b05f83bc5e793c1cd67b148a4b680467b5a5730ab1a31fe4e6460ee9f`; `/opt/homebrew/bin/gh` SHA `02d2d4a85241c6a8c0b77ebb1ec76fc723caf7fb128e00915b306b968847cba1` / `gh version 2.96.0 (2026-07-02)`。Any drift STOPs。
  - s1_recovery_push_invocation: Executor argv is exact `/bin/bash`,`--noprofile`,`--norc`,`-c`, `<script>` with `BASH_ENV|ENV|SHELLOPTS|BASHOPTS|CDPATH|GLOBIGNORE|PROMPT_COMMAND` absent。Script is ASCII, no LF/NUL, exact grammar `exec /usr/bin/env -i ` followed by each env assignment and argv entry shell-single-quoted in fixed order; values may contain printable ASCII including spaces/`!` but never single-quote/NUL/LF, so no quote escape or expansion exists; no prefix/suffix command。Script SHA is over exact bytes。Expanded git argv exact ordered entries are `/usr/bin/git`, `-c`, `credential.https://github.com.helper=`, `-c`, `credential.https://github.com.helper=!/opt/homebrew/bin/gh auth git-credential`, `-c`, `http.followRedirects=false`, `-c`, `http.extraHeader=`, `-c`, `push.followTags=false`, `-c`, `push.gpgSign=false`, `-c`, `push.useForceIfIncludes=false`, `push`, `--no-verify`, `--no-follow-tags`, `--recurse-submodules=no`, `--atomic`, `--force-with-lease=refs/heads/main:<base>`, `https://github.com/yusuketakuma/yrese.git`, `<commit>:refs/heads/main`。Minimal env exact ordered entries are `HOME=/Users/yusuke`,`PATH=/usr/bin:/bin:/usr/sbin:/sbin:/opt/homebrew/bin`,`LC_ALL=C`,`LANG=C`,`GIT_CONFIG_NOSYSTEM=1`,`GIT_CONFIG_SYSTEM=/dev/null`,`GIT_CONFIG_GLOBAL=/dev/null`,`GIT_EXEC_PATH=<pinned>`,`GIT_TERMINAL_PROMPT=0`,`GH_CONFIG_DIR=/Users/yusuke/.config/gh`; no others。`/bin/sh` used internally by the exact credential-helper config is pinned above。Local repo config must equal exact ten-record whitelist manifest SHA `75bd4d0afc3d17ca2c63197342ab543829deb1a02f387ff6a929e8ba4d1d3107`; worktree config absent; all include/url/http/push/hook config outside command overrides forbidden。Client hooks/tags/submodules/redirects are bounded; server hooks/webhooks/Actions are expected external effects and nonclaimed。
  - s1_recovery_remote_snapshot: Pinned sanitized `ls-remote --refs` is capped 1MiB/10,000 lines/final LF; each line is exact lowercase40hex + TAB + `git check-ref-format` valid ref, duplicate ref/OID pair fails, raw records byte-sort/LF/final LF。Remove exact singleton main; all other advertised namespaces remain。Current non-target manifest is exact 58-byte `f4d0f8f71a62c3e55a57a79ad57092b10620cc70<TAB>refs/pull/1/head<LF>` / SHA `4f3185217ec6f2309bab38a8fdbf25112b8f141204b139449686f993039899f2`。PLAN/post-AC9L/prepush and post-attempt bytes/digest must match. PLAN local=base; post-AC9L/prepush local=one-parent commit while remote main=base。
  - s1_recovery_human_approval: Literal version `WP4158_FORCE_LEASE_APPROVAL_V1`; nonce exact64 lowercase hex/32 CSPRNG bytes; UTC expiry exact valid `YYYY-MM-DDTHH:MM:SSZ`, >issue and ≤600s, bounded also by monotonic deadline。Canonical ordered `name NUL value LF` fields are `approvalVersion,nonce,base,commit,remote,ref,toolManifestSha256,executorArgvSha256,wrapperSha256,argvSha256,envSha256,nonTargetRefsSha256,expiresAtUtc`; no extras/NUL/LF。Argv hashes use every UTF-8 entry + NUL including terminal NUL; env uses ordered `key NUL value LF`; wrapper hashes exact no-LF script。Human token exact `APPROVE_WP4158_FORCE_LEASE_V1 request=<requestSha> nonce=<nonce> base=<base> commit=<commit> refs=<refsSha> tools=<toolSha> executor=<executorSha> wrapper=<wrapperSha> argv=<argvSha> env=<envSha> expires=<utc>` from a new authenticated current-user host turn。Request SHA/turn ID/receipt/expiry are recorded; missing host identity STOP。
  - s1_recovery_approval_claim: Private parent `/Users/yusuke/.codex/run` mode0700。After approval, exact nonce state file mode0600 stores V1 ordered manifest fields request/turn/status=`APPROVED`/expiry, fsync file+parent。Contenders must atomically `mkdir` nonce-specific `.claim` mode0700 with no-replace; exactly one succeeds and no stale/liveness reclaim is ever allowed。Winner writes a fresh64hex attempt ID and `CONSUMED` manifest with request/turn/claimed UTC/status before spawn, fsyncs file+claim+parent; loser/resume seeing claim or CONSUMED may only classify remote。A controlled two-contender guard requires one claim/spawn authority and one classification-only loser。Crash at any point after claim can never retry; state contains base/commit/ref/ref-sha/attempt needed for classification。Expiry/drift before claim invalidates APPROVED; state files contain no credential。
  - s1_recovery_push_state_machine: Pinned bash wrapper launches one child with `start_new_session=True`, verifies child PGID, caps stdout/stderr 1MiB each, deadline120s。Timeout sends TERM only to verified child group, waits2s, KILLs same group if needed, reaps; all exit/timeout/transport results then take a fresh pinned all-ref snapshot。Exit0 + main=commit/refs same => `PUSH_APPLIED`; nonzero/timeout/response-loss + same => `ALREADY_PUSHED_RECOVERY`; main=base/refs same => `PUSH_NOT_APPLIED`; main missing/other or refs drift => `REMOTE_DIVERGED`; unreadable/schema-invalid => `REMOTE_STATE_UNKNOWN`。Only first two proceed; never retry。
  - s1_recovery_ac9p: Pinned gh runs under a separately fixed minimal env, isolated PGID, stdout/stderr cap1MiB, per-call15s supervisor with TERM2s/KILL2s/reap。Exact GET argv targets `/repos/yusuketakuma/yrese/actions/workflows/309812329/runs?event=push&branch=main&head_sha=<commit>&per_page=100`, `/repos/yusuketakuma/yrese/actions/runs/<run>/jobs?per_page=100&page=<n>`, and `/repos/yusuketakuma/yrese/deployments?sha=<commit>&per_page=100`。Poll deadline1200s/interval15/max80。Run response object has `total_count` integer and `workflow_runs` list; filter exact workflow ID/path `.github/workflows/ci.yml`, event push, head_branch main, head_sha commit, run_attempt1; exactly one stable run ID, status completed/conclusion success。Jobs pages require object `total_count` and jobs list, positive total, unique integer IDs, page accumulation exactly total then empty next page; every job conclusion success and nonempty steps list, every step conclusion success。Deployments response exact empty list and no next page。Request/JSON/schema/pagination/deadline error STOP。Terminal token exact `WP4158_RECOVERY_PUSH_PASS commit=<commit> base=<base> remote=https://github.com/yusuketakuma/yrese.git ref=refs/heads/main refs=4f3185217ec6f2309bab38a8fdbf25112b8f141204b139449686f993039899f2 run=<run> workflow=309812329 attempt=1 deployments=0` after local/pinned-main parity and clean state。S2 begins only from terminal landed recovery commit。
  - s1_recovery_v2_precedence: The following V2 clauses replace the preceding V1 recovery push runtime/invocation/snapshot/approval/claim/state-machine/AC9P clauses in full. V1 must not be combined with V2 and cannot authorize a mutation。
  - s1_recovery_v2_manifests: Tool Manifest V2 is the following exact 20 ordered `name NUL value LF` records, no sorting/extras, SHA-256 `ce5be2cadb31d0c90f56538e201062b028bce9c8a063864dd733379b42695678`: `bash.path=/bin/bash`; `bash.sha256=fde343ee184953c1fa1185abddeaa8be61c6acbebae4eb54db5d6b55b09a5755`; `bash.version=GNU bash, version 3.2.57(1)-release (arm64-apple-darwin25)`; `env.path=/usr/bin/env`; `env.sha256=6e506aec3c0cff703ac1e66cedc6f1945354ad41339a38db4425c7c88227128f`; `git.path=/usr/bin/git`; `git.sha256=179301dcb41ea78accc3fa0048a7e6f6710d891945a751a34addd622020c1818`; `git.version=git version 2.50.1 (Apple Git-155)`; `git.execPath=/Library/Developer/CommandLineTools/usr/libexec/git-core`; `gitRemoteHttps.path=/Library/Developer/CommandLineTools/usr/libexec/git-core/git-remote-https`; `gitRemoteHttps.sha256=badb3dc6666bdffdc3c6eaf44346eb390bb43d69b1608da4d1f971bbdb2d2998`; `sh.path=/bin/sh`; `sh.sha256=ad5c194b05f83bc5e793c1cd67b148a4b680467b5a5730ab1a31fe4e6460ee9f`; `gh.path=/opt/homebrew/bin/gh`; `gh.sha256=02d2d4a85241c6a8c0b77ebb1ec76fc723caf7fb128e00915b306b968847cba1`; `gh.version=gh version 2.96.0 (2026-07-02)`; `pythonSystem.invocation=/usr/bin/python3`; `pythonSystem.sysExecutable=/Library/Developer/CommandLineTools/usr/bin/python3`; `pythonSystem.realpath=/Library/Developer/CommandLineTools/Library/Frameworks/Python3.framework/Versions/3.9/bin/python3.9`; `pythonSystem.version=Python 3.9.6`。Local Config V2 is exact ten ordered `key NUL value LF` records, no sorting/extras, SHA-256 `02146ac58e289564b567bde31586f03b35a9c9b404d8eff056595bdbdeee58f7`: `core.repositoryformatversion=0`; `core.filemode=true`; `core.bare=false`; `core.logallrefupdates=true`; `core.ignorecase=true`; `core.precomposeunicode=true`; `remote.origin.url=https://github.com/yusuketakuma/yrese.git`; `remote.origin.fetch=+refs/heads/*:refs/remotes/origin/*`; `branch.main.remote=origin`; `branch.main.merge=refs/heads/main`。Read via pinned git with system/global config disabled; duplicate key、worktree config、include/includeIf、url/http/push/hooks/credential config outside exact command overrides are forbidden。Any byte/runtime/config mode-owner-path drift STOPs。
  - s1_recovery_v2_supervisor: Recovery implementation emits one canonical extracted supervisor source inside the inline verifier; its full-file SHA/LF count become `SUPERVISOR_SOURCE_SHA256/LF_COUNT` at IMPLEMENTATION and are rechecked from committed S1 recovery bytes before approval and execution。Only exact `/usr/bin/python3 -I <owned-source0600> --recovery-push <approved-packet0600>` may interpret it; both files live in one private mode0700 `/Users/yusuke/.codex/run/wp4158-<nonce>` directory, are opened no-follow/identity-checked, and source bytes must equal the approved SHA。Supervisor argv hash is every UTF-8 entry plus terminal NUL。The same source exclusively implements approval validation, atomic claim, `start_new_session=True`, PGID verification, capped nonblocking output, deadline/TERM/KILL/reap, remote classification, gh pagination and residue checks; no shell/Python snippet, imported repo module, resume helper or second implementation may mutate/classify。Static guards alter each supervisor call site and runtime guards cover no-spawn invalid approval, two contenders, every claim crash cut, timeout kill/reap, cap overflow, unreadable remote and pagination drift。
  - s1_recovery_v2_remote: The pinned supervisor runs exact argv `/usr/bin/git`,`-c`,`credential.helper=`,`-c`,`http.followRedirects=false`,`-c`,`http.extraHeader=`,`ls-remote`,`--refs`,`https://github.com/yusuketakuma/yrese.git` under exact ordered env `HOME=/Users/yusuke`,`PATH=/usr/bin:/bin:/usr/sbin:/sbin`,`LC_ALL=C`,`LANG=C`,`GIT_CONFIG_NOSYSTEM=1`,`GIT_CONFIG_SYSTEM=/dev/null`,`GIT_CONFIG_GLOBAL=/dev/null`,`GIT_EXEC_PATH=/Library/Developer/CommandLineTools/usr/libexec/git-core`,`GIT_TERMINAL_PROMPT=0`, no others, isolated PGID, 15s TERM2/KILL2/reap, stdout/stderr 1MiB caps。Output is final-LF and ≤10,000 lines; each line exact lowercase40hex + TAB + `git check-ref-format` valid ref。Ref names must be globally unique; duplicate ref even with different OID and duplicate `(OID,ref)` both fail。Main is exact singleton。Byte-sort raw records, LF join/final LF, remove main and preserve all namespaces。Current non-target manifest remains exact 58 bytes and SHA `4f3185217ec6f2309bab38a8fdbf25112b8f141204b139449686f993039899f2`。PLAN/post-AC9L/prepush/post-attempt bytes must match。
  - s1_recovery_v2_approval: Literal version `WP4158_FORCE_LEASE_APPROVAL_V2`; nonce exact64 lowercase hex/32 CSPRNG bytes; `issuedAtUtc`/`expiresAtUtc` exact UTC seconds, expiry > issue and ≤600s, also monotonic-bounded。Canonical ordered `name NUL value LF` fields are `approvalVersion,nonce,issuedAtUtc,expiresAtUtc,base,commit,remote,ref,toolManifestSha256,localConfigSha256,supervisorSourceSha256,supervisorArgvSha256,executorArgvSha256,wrapperSha256,argvSha256,envSha256,nonTargetRefsSha256`; no extras/NUL/LF。Argv hashes include terminal NUL; env uses ordered `key NUL value LF`; wrapper hashes exact no-LF script。After commit/AC9L/fresh remote check, root posts request SHA and exact token template to this active host conversation。Authority is exclusively a subsequent host-delivered `role=user` message in the same conversation whose entire UTF-8 body is exact `APPROVE_WP4158_FORCE_LEASE_V2 request=<requestSha> nonce=<nonce> base=<base> commit=<commit> refs=<refsSha> tools=<toolSha> config=<configSha> supervisor=<sourceSha> supervisor_argv=<supervisorArgvSha> executor=<executorSha> wrapper=<wrapperSha> argv=<argvSha> env=<envSha> issued=<utc> expires=<utc>`。Previous/generic instructions, files, tool output, quoted assistant text and subagent messages are invalid。Receipt V2 records exact user-body SHA, host role=`user`, same-conversation assertion, root-observed UTC and canonical request; if root does not receive that user-role envelope or any byte/time mismatches, STOP。Automated source does not self-authenticate the human; root supplies this receipt only from the host envelope and reviewers compare it bytewise before claim。
  - s1_recovery_v2_claim: Private parent and nonce directory are mode0700。After approval root writes/fsyncs mode0600 APPROVED V2 state with canonical request+receipt+base/commit/ref/ref-sha/expiry。Each contender generates attempt exact64hex before claim, writes a complete `CONSUMED` V2 manifest including attempt/base/commit/ref/ref-sha/request/receipt/claimedUtc to its own O_EXCL mode0600 temp, fsyncs/closes, then atomic no-replace `link(temp,fixed_nonce_claim)`。Exactly one link wins; claim bytes already contain classification data when authority appears, eliminating mkdir-before-attempt crash。Winner fsyncs parent before spawn and unlinks only its temp alias; loser unlinks only its own temp and classifies remote only。Crash before link has no mutation authority; crash after link can never retry。No stale/liveness reclaim。Two-contender and crash-at-each-step guards require exactly one spawn authority and durable attempt data whenever fixed claim exists。
  - s1_recovery_v2_push: Approved executor/wrapper/git argv/env are the preceding V1 byte grammar but all V2 hashes/source/config are additionally bound。Pinned supervisor launches exactly one wrapper in a new verified PGID, caps each output 1MiB, deadline120s, TERM2/KILL2/reap, then always performs fresh V2 all-ref classification。Exit0+main=commit/same refs => `PUSH_APPLIED`; nonzero/timeout/response-loss+same => `ALREADY_PUSHED_RECOVERY`; main=base/same => `PUSH_NOT_APPLIED`; missing/other main or refs drift => `REMOTE_DIVERGED`; unreadable/invalid => `REMOTE_STATE_UNKNOWN`; only first two proceed and never retry。Literal-URL push does not update `refs/remotes/origin/main`; that tracking ref is non-authoritative and may remain base。Terminal parity is local `HEAD`/commit object against pinned remote `refs/heads/main`, never origin tracking-ref parity。
  - s1_recovery_v2_ac9p: Same pinned supervisor runs exact argv `/opt/homebrew/bin/gh`,`api`,`--hostname`,`github.com`,`--method`,`GET`,`--header`,`Accept: application/vnd.github+json`,`--header`,`X-GitHub-Api-Version: 2022-11-28`,`<endpoint>` under exact ordered env `HOME=/Users/yusuke`,`PATH=/usr/bin:/bin:/usr/sbin:/sbin:/opt/homebrew/bin`,`LC_ALL=C`,`LANG=C`,`GH_CONFIG_DIR=/Users/yusuke/.config/gh`,`GH_HOST=github.com`,`GH_PROMPT_DISABLED=1`,`NO_COLOR=1`, no others, isolated PGID, 1MiB caps, per-call15s TERM2/KILL2/reap。Workflow endpoints `/repos/yusuketakuma/yrese/actions/workflows/309812329/runs?event=push&branch=main&head_sha=<commit>&per_page=100&page=<n>` enumerate n=1 through required empty page; every response has stable nonnegative integer `total_count`, list `workflow_runs`, unique integer IDs, accumulated count exactly total before empty next page。Complete accumulation contains exactly one matching workflow ID/path `.github/workflows/ci.yml`, event push, head_branch main, head_sha commit, run_attempt1, stable run ID, completed/success。Jobs endpoint `/repos/yusuketakuma/yrese/actions/runs/<run>/jobs?per_page=100&page=<n>` uses same stable-total/unique-ID/exact-count/empty-next rules, positive total, all jobs/steps success and steps nonempty。Deployments exact page1 endpoint `/repos/yusuketakuma/yrese/deployments?sha=<commit>&per_page=100&page=1` must return exact empty list; empty page1 proves no later page。Poll deadline1200s/interval15/max80 across all calls。Any request/JSON/schema/pagination/deadline drift STOP。Terminal token remains the preceding exact token after local HEAD/pinned remote main parity and clean state; origin tracking-ref parity is not required。
  - s1_recovery_v3_precedence: The following V3 clauses replace V1 and V2 recovery push clauses in full. V3 is standalone; V1/V2 bytes, hashes, grammar and tokens are historical non-authority and must not be referenced, imported or combined。
  - s1_recovery_v3_tool_manifest: Tool Manifest V3 is these exact 22 ordered `name NUL value LF` records, no sorting/extras, 1,364 bytes, SHA-256 `54513a0642ce49b294f5db1de1caf84586b2eb26ec89ffec94eb10e72f7cd021`: `bash.path=/bin/bash`; `bash.sha256=fde343ee184953c1fa1185abddeaa8be61c6acbebae4eb54db5d6b55b09a5755`; `bash.version=GNU bash, version 3.2.57(1)-release (arm64-apple-darwin25)`; `env.path=/usr/bin/env`; `env.sha256=6e506aec3c0cff703ac1e66cedc6f1945354ad41339a38db4425c7c88227128f`; `git.path=/usr/bin/git`; `git.sha256=179301dcb41ea78accc3fa0048a7e6f6710d891945a751a34addd622020c1818`; `git.version=git version 2.50.1 (Apple Git-155)`; `git.execPath=/Library/Developer/CommandLineTools/usr/libexec/git-core`; `gitRemoteHttps.path=/Library/Developer/CommandLineTools/usr/libexec/git-core/git-remote-https`; `gitRemoteHttps.sha256=badb3dc6666bdffdc3c6eaf44346eb390bb43d69b1608da4d1f971bbdb2d2998`; `sh.path=/bin/sh`; `sh.sha256=ad5c194b05f83bc5e793c1cd67b148a4b680467b5a5730ab1a31fe4e6460ee9f`; `gh.path=/opt/homebrew/bin/gh`; `gh.sha256=02d2d4a85241c6a8c0b77ebb1ec76fc723caf7fb128e00915b306b968847cba1`; `gh.version=gh version 2.96.0 (2026-07-02)`; `pythonSystem.invocation=/usr/bin/python3`; `pythonSystem.invocationSha256=179301dcb41ea78accc3fa0048a7e6f6710d891945a751a34addd622020c1818`; `pythonSystem.sysExecutable=/Library/Developer/CommandLineTools/usr/bin/python3`; `pythonSystem.realpath=/Library/Developer/CommandLineTools/Library/Frameworks/Python3.framework/Versions/3.9/bin/python3.9`; `pythonSystem.realpathSha256=4b42b1a117605cafc8607b67b0892a609c2cd125012dd56288abeed8c89cdfb1`; `pythonSystem.version=Python 3.9.6`。Both invocation and resolved interpreter bytes are human-bound; any drift STOPs。
  - s1_recovery_v3_execution_context: Before approval, root creates `/Users/yusuke/.codex/run` if absent using mode0700 under existing canonical no-symlink `/Users/yusuke/.codex` directory mode0700 uid501 gid20; it never replaces an existing object。V3 context manifest uses exact ordered `name NUL value LF`: `cwd=/`; worktree path `/Users/yusuke/workspace/yrese`, type directory, mode0755, uid501, gid20, dev16777233, ino239899247; gitdir path `/Users/yusuke/workspace/yrese/.git`, type directory, mode0755, uid501, gid20, dev16777233, ino239900068; config path `/Users/yusuke/workspace/yrese/.git/config`, type regular, mode0644, uid501, gid20, dev16777233, ino267521965; codexParent path `/Users/yusuke/.codex`, type directory, mode0700, uid501, gid20, dev16777233, ino628625; and runParent canonical path/type=directory/mode0700/uid501/gid20/current dev+ino captured after safe creation。Every component is `lstat` checked, symlink forbidden, canonical path exact; file/dir descriptors use no-follow and fstat identity. The complete context manifest SHA is `executionContextSha256` in request/approval and is rederived at every local check。Supervisor and every child use cwd `/`; local Git commands carry exact `--git-dir=/Users/yusuke/workspace/yrese/.git`,`--work-tree=/Users/yusuke/workspace/yrese`, so repository discovery and ambient cwd cannot select another object database。
  - s1_recovery_v3_config_read: Local Config V3 keeps the exact ten records and SHA `02146ac58e289564b567bde31586f03b35a9c9b404d8eff056595bdbdeee58f7` from V2, now read only by exact argv `/usr/bin/git`,`config`,`--file`,`/Users/yusuke/workspace/yrese/.git/config`,`--no-includes`,`--null`,`--list` under cwd `/` and exact env `HOME=/Users/yusuke`,`PATH=/usr/bin:/bin:/usr/sbin:/sbin`,`LC_ALL=C`,`LANG=C`,`GIT_CONFIG_NOSYSTEM=1`,`GIT_CONFIG_SYSTEM=/dev/null`,`GIT_CONFIG_GLOBAL=/dev/null`,`GIT_TERMINAL_PROMPT=0`, no others, isolated PGID, 5s TERM2/KILL2/reap, stdout/stderr caps1MiB。Parse exact repeated `key LF value NUL`, require final NUL, UTF-8/no embedded NUL or LF in key/value, lowercase key, no duplicates, then emit the ten ordered `key NUL value LF` authority records; output order and values must match exactly。Config/context lstat+fstat identities must match before and after read。
  - s1_recovery_v3_supervisor_and_remote: Implementation emits one canonical supervisor source inside the committed inline verifier; full source SHA/LF count and exact argv `/usr/bin/python3`,`-I`,`<owned-source0600>`,`--recovery-push`,`<approved-packet0600>` plus cwd/context hashes are human-bound。Only that source/runtime implements validation, atomic claim, process supervision, mutation, classification and AC9P; static/runtime guards cover every call site/failure cut。Remote read uses exact git argv `/usr/bin/git`,`-c`,`credential.helper=`,`-c`,`http.followRedirects=false`,`-c`,`http.extraHeader=`,`ls-remote`,`--refs`,`https://github.com/yusuketakuma/yrese.git`, cwd `/`, exact ordered env `HOME=/Users/yusuke`,`PATH=/usr/bin:/bin:/usr/sbin:/sbin`,`LC_ALL=C`,`LANG=C`,`GIT_CONFIG_NOSYSTEM=1`,`GIT_CONFIG_SYSTEM=/dev/null`,`GIT_CONFIG_GLOBAL=/dev/null`,`GIT_EXEC_PATH=/Library/Developer/CommandLineTools/usr/libexec/git-core`,`GIT_TERMINAL_PROMPT=0`, no others, PGID15s/TERM2/KILL2/reap/1MiB caps。Each output ref is UTF-8/lowercase40hex/TAB/final-LF, ≤10,000 unique ref names and pairs, main singleton。V3 validates each ref by this exact pure rule: starts `refs/`; total UTF-8 byte length 6..1024; at least two slash-separated nonempty components; no component starts dot or ends `.lock` case-insensitively; ref does not end dot or slash; contains no `..`, `@{`, `//`, ASCII control00-1f/7f, space, `~`, `^`, `:`, `?`, `*`, `[`, or backslash。Byte-sort/raw LF/final LF, remove main, preserve all namespaces; current non-target exact58/SHA `4f3185217ec6f2309bab38a8fdbf25112b8f141204b139449686f993039899f2` must match PLAN/preclaim/post-attempt/terminal。
  - s1_recovery_v3_executor: Executor argv is exact `/bin/bash`,`--noprofile`,`--norc`,`-c`,`<script>`。Script exact bytes are ASCII/no LF/NUL: literal `exec /usr/bin/env -i ` followed by each push-env assignment then git argv entry, each enclosed in one ASCII single-quote, separated by one space, no prefix/suffix; every value is printable ASCII but contains no single-quote/NUL/LF, so no expansion/escape。Push env exact ordered entries: `HOME=/Users/yusuke`,`PATH=/usr/bin:/bin:/usr/sbin:/sbin:/opt/homebrew/bin`,`LC_ALL=C`,`LANG=C`,`GIT_CONFIG_NOSYSTEM=1`,`GIT_CONFIG_SYSTEM=/dev/null`,`GIT_CONFIG_GLOBAL=/dev/null`,`GIT_EXEC_PATH=/Library/Developer/CommandLineTools/usr/libexec/git-core`,`GIT_TERMINAL_PROMPT=0`,`GH_CONFIG_DIR=/Users/yusuke/.config/gh`; no others and dangerous bash env names absent。Git argv exact ordered entries: `/usr/bin/git`,`--git-dir=/Users/yusuke/workspace/yrese/.git`,`--work-tree=/Users/yusuke/workspace/yrese`,`-c`,`credential.https://github.com.helper=`,`-c`,`credential.https://github.com.helper=!/opt/homebrew/bin/gh auth git-credential`,`-c`,`http.followRedirects=false`,`-c`,`http.extraHeader=`,`-c`,`push.followTags=false`,`-c`,`push.gpgSign=false`,`-c`,`push.useForceIfIncludes=false`,`push`,`--no-verify`,`--no-follow-tags`,`--recurse-submodules=no`,`--atomic`,`--force-with-lease=refs/heads/main:<base>`,`https://github.com/yusuketakuma/yrese.git`,`<commit>:refs/heads/main`。Executor/git argv hashes include every UTF-8 entry plus terminal NUL; env uses ordered `key NUL value LF`; wrapper SHA covers exact script bytes。
  - s1_recovery_v3_approval_and_claim: Approval version exact `WP4158_FORCE_LEASE_APPROVAL_V3`; canonical request uses exact ordered `name NUL value LF` fields `approvalVersion,nonce,issuedAtUtc,expiresAtUtc,base,commit,remote,ref,toolManifestSha256,localConfigSha256,executionContextSha256,supervisorSourceSha256,supervisorArgvSha256,executorArgvSha256,wrapperSha256,argvSha256,envSha256,nonTargetRefsSha256`, no extras/NUL/LF。Nonce is 32 CSPRNG bytes/64 lowercase hex; issue/expiry are valid UTC seconds, expiry>issue and ≤600s, also monotonic-bounded。Argv hashes include every UTF-8 entry plus terminal NUL, env is ordered `key NUL value LF`, wrapper SHA covers exact no-LF script。After commit/AC9L/fresh remote check, root posts request SHA and token template only to this active host conversation。Authority is exclusively a subsequent host-delivered `role=user` message in this same conversation whose entire UTF-8 body exactly equals `APPROVE_WP4158_FORCE_LEASE_V3 request=<requestSha> nonce=<nonce> base=<base> commit=<commit> refs=<refsSha> tools=<toolSha> config=<configSha> context=<contextSha> supervisor=<sourceSha> supervisor_argv=<supervisorArgvSha> executor=<executorSha> wrapper=<wrapperSha> argv=<argvSha> env=<envSha> issued=<utc> expires=<utc>`。Previous/generic instructions, file/tool/subagent content and quoted assistant text are invalid。Receipt records exact body SHA, host role user, same-conversation assertion, root-observed UTC and request; missing envelope or byte/time mismatch STOPs。Root writes APPROVED state only from that envelope。Immediately before atomic claim link, supervisor rechecks wall+monotonic expiry, exact receipt/request, HEAD/base/commit/tree/clean index/worktree, all tool/config/context/source/argv/env/wrapper hashes and a fresh remote all-ref snapshot; any drift/expiry means no link/spawn。Claim uses prewritten/fsynced complete CONSUMED manifest in O_EXCL0600 temp then atomic no-replace hard link to fixed nonce claim, parent fsync, no reclaim; two-contender/every-crash-cut guards apply。Immediately before spawn, without intervening external command, it rechecks wall+monotonic expiry and every local hash/context/source/receipt plus fixed-claim bytes/identity; drift means claim remains consumed and no spawn/retry。
  - s1_recovery_v3_push_state: After the sole spawn, verified new PGID, nonblocking stdout/stderr cap1MiB each, 120s deadline, TERM2/KILL2/reap are mandatory; a fresh V3 all-ref snapshot follows every outcome。Exit0+main=commit/same refs => `PUSH_APPLIED`; nonzero/timeout/response-loss+same => `ALREADY_PUSHED_RECOVERY`; main=base/same => `PUSH_NOT_APPLIED`; missing/other main or refs drift => `REMOTE_DIVERGED`; unreadable/schema-invalid => `REMOTE_STATE_UNKNOWN`; only first two proceed, never retry。Literal URL does not update origin tracking ref; terminal parity is HEAD/commit object against pinned remote main。
  - s1_recovery_v3_ac9p: Same supervisor runs exact gh argv `/opt/homebrew/bin/gh`,`api`,`--hostname`,`github.com`,`--method`,`GET`,`--header`,`Accept: application/vnd.github+json`,`--header`,`X-GitHub-Api-Version: 2022-11-28`,`<endpoint>` under exact ordered env `HOME=/Users/yusuke`,`PATH=/usr/bin:/bin:/usr/sbin:/sbin:/opt/homebrew/bin`,`LC_ALL=C`,`LANG=C`,`GH_CONFIG_DIR=/Users/yusuke/.config/gh`,`GH_HOST=github.com`,`GH_PROMPT_DISABLED=1`,`NO_COLOR=1`, no others, cwd `/`, PGID/per-call15s/TERM2/KILL2/reap/1MiB caps。Every poll exhaustively enumerates workflow pages n=1 through empty next page with stable total, unique IDs and exact accumulated count; matching key is workflow309812329/path`.github/workflows/ci.yml`/push/main/head commit/attempt1。Before first match, zero matches means wait; first exactly-one queued/in_progress with null conclusion pins run ID and waits; same pinned run may remain queued/in_progress/null; completed/success proceeds; completed with any other conclusion or any impossible status/conclusion is terminal failure; multiple matches, disappearance after pin, or run-ID drift STOP。Deadline1200s, interval15s, max80 poll cycles。After success, jobs pages use exact total/unique IDs/empty next page and require positive all-success jobs/nonempty all-success steps; deployments page1 must be exact empty list。Immediately before terminal token, take another fresh V3 all-ref snapshot and require main=commit plus exact non-target bytes/SHA, then exact local HEAD/commit/tree/clean state。Terminal token is standalone exact `WP4158_RECOVERY_PUSH_PASS commit=<commit> base=<base> remote=https://github.com/yusuketakuma/yrese.git ref=refs/heads/main refs=4f3185217ec6f2309bab38a8fdbf25112b8f141204b139449686f993039899f2 run=<run> workflow=309812329 attempt=1 deployments=0`。Any request/schema/pagination/deadline/parity drift STOPs。
  - s1_recovery_v4_precedence: The following V4 clauses replace every V1/V2/V3 recovery push clause in full. V4 is standalone and must not import prior recovery-push text。
  - s1_recovery_v4_tools_and_config: Tool Manifest V4 is exact 22 ordered `name NUL value LF` records, no sorting/extras, 1,364 bytes, SHA `54513a0642ce49b294f5db1de1caf84586b2eb26ec89ffec94eb10e72f7cd021`: `bash.path=/bin/bash`; `bash.sha256=fde343ee184953c1fa1185abddeaa8be61c6acbebae4eb54db5d6b55b09a5755`; `bash.version=GNU bash, version 3.2.57(1)-release (arm64-apple-darwin25)`; `env.path=/usr/bin/env`; `env.sha256=6e506aec3c0cff703ac1e66cedc6f1945354ad41339a38db4425c7c88227128f`; `git.path=/usr/bin/git`; `git.sha256=179301dcb41ea78accc3fa0048a7e6f6710d891945a751a34addd622020c1818`; `git.version=git version 2.50.1 (Apple Git-155)`; `git.execPath=/Library/Developer/CommandLineTools/usr/libexec/git-core`; `gitRemoteHttps.path=/Library/Developer/CommandLineTools/usr/libexec/git-core/git-remote-https`; `gitRemoteHttps.sha256=badb3dc6666bdffdc3c6eaf44346eb390bb43d69b1608da4d1f971bbdb2d2998`; `sh.path=/bin/sh`; `sh.sha256=ad5c194b05f83bc5e793c1cd67b148a4b680467b5a5730ab1a31fe4e6460ee9f`; `gh.path=/opt/homebrew/bin/gh`; `gh.sha256=02d2d4a85241c6a8c0b77ebb1ec76fc723caf7fb128e00915b306b968847cba1`; `gh.version=gh version 2.96.0 (2026-07-02)`; `pythonSystem.invocation=/usr/bin/python3`; `pythonSystem.invocationSha256=179301dcb41ea78accc3fa0048a7e6f6710d891945a751a34addd622020c1818`; `pythonSystem.sysExecutable=/Library/Developer/CommandLineTools/usr/bin/python3`; `pythonSystem.realpath=/Library/Developer/CommandLineTools/Library/Frameworks/Python3.framework/Versions/3.9/bin/python3.9`; `pythonSystem.realpathSha256=4b42b1a117605cafc8607b67b0892a609c2cd125012dd56288abeed8c89cdfb1`; `pythonSystem.version=Python 3.9.6`。Local Config V4 exact ten ordered `key NUL value LF` records/no extras/SHA `02146ac58e289564b567bde31586f03b35a9c9b404d8eff056595bdbdeee58f7`: `core.repositoryformatversion=0`; `core.filemode=true`; `core.bare=false`; `core.logallrefupdates=true`; `core.ignorecase=true`; `core.precomposeunicode=true`; `remote.origin.url=https://github.com/yusuketakuma/yrese.git`; `remote.origin.fetch=+refs/heads/*:refs/remotes/origin/*`; `branch.main.remote=origin`; `branch.main.merge=refs/heads/main`。
  - s1_recovery_v4_context: Root safely creates absent `/Users/yusuke/.codex/run` mode0700 without replacing an object under canonical no-symlink `/Users/yusuke/.codex` mode0700 uid501 gid20。Context V4 exact ordered `name NUL value LF` names are `cwd`; then for each prefix `worktree,gitdir,config,codexParent,runParent` exactly `.path,.type,.mode,.uid,.gid,.dev,.ino`。Values: `cwd=/`; worktree `/Users/yusuke/workspace/yrese,directory,0755,501,20,16777233,239899247`; gitdir `/Users/yusuke/workspace/yrese/.git,directory,0755,501,20,16777233,239900068`; config `/Users/yusuke/workspace/yrese/.git/config,regular,0644,501,20,16777233,267521965`; codexParent `/Users/yusuke/.codex,directory,0700,501,20,16777233,628625`; runParent path `/Users/yusuke/.codex/run`, type directory, mode0700, uid501, gid20, with decimal no-leading-zero current `dev,ino` captured after creation。Types/modes are exact displayed ASCII; all other numeric values decimal ASCII。Every path component uses lstat, forbids symlink, exact canonical path; opened objects use no-follow and fstat equality。The 36-record context SHA is computed after runParent creation, included in approval, and rederived at each check; any fixed or captured identity drift STOPs。
  - s1_recovery_v4_config_and_local_git: Config read exact argv `/usr/bin/git`,`config`,`--file`,`/Users/yusuke/workspace/yrese/.git/config`,`--no-includes`,`--null`,`--list`; parse repeated UTF-8 `key LF value NUL`/final NUL, lowercase key/no embedded LF-NUL/no duplicate, then require exact V4 manifest order/bytes/SHA。All local Git checks use cwd `/`, exact env `HOME=/Users/yusuke`,`PATH=/usr/bin:/bin:/usr/sbin:/sbin`,`LC_ALL=C`,`LANG=C`,`GIT_CONFIG_NOSYSTEM=1`,`GIT_CONFIG_SYSTEM=/dev/null`,`GIT_CONFIG_GLOBAL=/dev/null`,`GIT_TERMINAL_PROMPT=0`, no others, and command prefix `/usr/bin/git`,`--git-dir=/Users/yusuke/workspace/yrese/.git`,`--work-tree=/Users/yusuke/workspace/yrese`。Exact suffix/output: `rev-parse`,`--verify`,`HEAD^{commit}` => `<commit>\n`; `rev-list`,`--parents`,`-n`,`1`,`<commit>` => `<commit> <base>\n`; `show`,`-s`,`--format=%T`,`<commit>` => `<tree>\n`; `show`,`-s`,`--format=%s`,`<commit>` => exact message+LF; `status`,`--porcelain=v1`,`--untracked-files=no` => empty; `diff-tree`,`--no-commit-id`,`--name-status`,`-r`,`<base>`,`<commit>` => exactly four `M<TAB>path<LF>` records byte-sorted for exact4。Each is isolated PGID, 15s, TERM2/KILL2/reap, caps1MiB, exit0/stderr empty; config read uses same supervision without gitdir prefix。Real index path from gitdir is regular no-symlink and ambient `GIT_INDEX_FILE` is absent by empty child env。
  - s1_recovery_v4_files_and_launcher: Nonce exact64 lowercase hex. Exact nonce directory `/Users/yusuke/.codex/run/wp4158-<nonce>` is created once mode0700 uid501 gid20 on same device as runParent; preexistence STOP。Exact leaf names: `supervisor.py`, `approval.packet`, `approved.state`, `claim.<attempt>.tmp`, fixed `claim`, `result.state`。All operations are dir-fd relative with O_NOFOLLOW where available, lstat/fstat regular-file identity, O_EXCL0600 creation, complete write loops, fsync file then directory; rename/replace forbidden。Committed inline extraction writes exact supervisor bytes to `supervisor.py`; approval canonical bytes to `approval.packet`; both hashes/identities rechecked。Root launches supervisor only as exact argv `/usr/bin/env`,`-i`,`HOME=/Users/yusuke`,`PATH=/usr/bin:/bin:/usr/sbin:/sbin`,`LC_ALL=C`,`LANG=C`,`/usr/bin/python3`,`-I`,`<exact-supervisor-path>`,`--recovery-push`,`<exact-packet-path>`, with Popen env empty, cwd `/`, new verified PGID and capped output/deadline/reap。No `PYTHON*`, loader or shell variable exists before interpreter。
  - s1_recovery_v4_approval: Canonical request exact ordered `name NUL value LF` fields: `approvalVersion,nonce,issuedAtUtc,expiresAtUtc,monotonicIssuedNs,monotonicDeadlineNs,base,commit,remote,ref,toolManifestSha256,localConfigSha256,executionContextSha256,supervisorSourceSha256,supervisorArgvSha256,executorArgvSha256,wrapperSha256,argvSha256,envSha256,nonTargetRefsSha256`。Version=`WP4158_FORCE_LEASE_APPROVAL_V4`; wall times exact UTC seconds, expiry>issue and duration1..600s; monotonic values decimal integers with `deadline-issued = durationSeconds*1000000000`, captured from root `time.monotonic_ns()` when request is sealed。Valid time requires both wall now within `[issue,expiry)` and monotonic now within `[issued,deadline)`; monotonic below issued (including reboot) or either clock outside range STOPs。After commit/AC9L/fresh remote check root posts request SHA and exact template; authority is only a subsequent same-conversation host `role=user` message whose whole UTF-8 body exactly equals `APPROVE_WP4158_FORCE_LEASE_V4 request=<requestSha> nonce=<nonce> base=<base> commit=<commit> refs=<refsSha> tools=<toolSha> config=<configSha> context=<contextSha> supervisor=<sourceSha> supervisor_argv=<supervisorArgvSha> executor=<executorSha> wrapper=<wrapperSha> argv=<argvSha> env=<envSha> issued=<utc> expires=<utc> mono_issued=<ns> mono_deadline=<ns>`。Generic/prior/file/tool/subagent/quoted text invalid。Receipt exact ordered `name NUL value LF` fields `receiptVersion,userBodySha256,hostRole,sameConversation,observedAtUtc,requestSha256`; exact values are `WP4158_HOST_RECEIPT_V1`, SHA of entire user body, `user`, `true`, root observation UTC seconds, and request SHA。Missing envelope/byte/time mismatch STOP。
  - s1_recovery_v4_state_and_claim: `approved.state` canonical ordered fields `stateVersion,status,requestSha256,userBodySha256,observedAtUtc,base,commit,remote,ref,nonTargetRefsSha256,expiresAtUtc,monotonicDeadlineNs` with exact version `WP4158_APPROVED_STATE_V4`/status `APPROVED`。Each contender first generates attempt64hex, writes/fsyncs its complete temp canonical fields `stateVersion,status,attempt,requestSha256,userBodySha256,base,commit,remote,ref,nonTargetRefsSha256,claimedAtUtc` with exact version `WP4158_CLAIM_STATE_V4`/status `CONSUMED`, then atomic no-replace hard-links same-directory temp to fixed `claim`; exactly one wins and fsyncs nonce dir before spawn。Loser unlinks only own temp and performs classification-only; fixed claim/resume never reclaims or spawns。Crash before link has no authority; fixed claim always already contains attempt/classification fields; crash after link is consumed/no retry。`result.state` is O_EXCL0600 canonical ordered fields `stateVersion,status,attempt,base,commit,remoteMain,nonTargetRefsSha256,observedAtUtc,pushExit,stdoutSha256,stderrSha256` with version `WP4158_RESULT_STATE_V4`; status is one of the five classification literals, attempt/base/commit/refs bind claim, remoteMain is lowercase40hex or `MISSING`/`UNKNOWN`, pushExit is signed decimal or `TIMEOUT`/`RESPONSE_LOSS`/`NOT_SPAWNED`, and output hashes are lowercase64hex or `UNAVAILABLE`。Winner or classification-only resume writes/fsyncs it once; preexistence must parse and equal a fresh classification or STOP, never replace。Controlled two-contender, every crash cut, orphan-temp ownership, winner/loser/resume and residue guards are required。
  - s1_recovery_v4_temporal_and_remote: Immediately before claim link, supervisor validates both clocks, receipt/request/state, exact local Git outputs, tool/config/context/source/launcher/executor/wrapper/argv/env hashes, then a fresh all-ref snapshot。Immediately after winning+fsync claim and directly before spawn it repeats both clocks, receipt/request/approved/claim bytes, all local Git outputs and all local hashes/identities; no external command occurs between final local check and spawn。Remote main/non-target snapshot from immediately preclaim is fixed for force-lease; any local drift after claim consumes/no-spawn/no-retry。Post-attempt and terminal remote reads are fresh。Remote exact argv `/usr/bin/git`,`-c`,`credential.helper=`,`-c`,`http.followRedirects=false`,`-c`,`http.extraHeader=`,`ls-remote`,`--refs`,`https://github.com/yusuketakuma/yrese.git`, cwd `/`, local-Git env plus `GIT_EXEC_PATH=/Library/Developer/CommandLineTools/usr/libexec/git-core` inserted before `GIT_TERMINAL_PROMPT`, PGID15s/caps1MiB。Parse ≤10,000 final-LF records lowercase40hex/TAB/ref; unique ref name/pair; main singleton。Ref pure rule: starts `refs/`, UTF-8 bytes6..1024, ≥2 nonempty slash components, no component starts dot or case-insensitive suffix `.lock`, no terminal dot/slash, no `..`,`@{`,`//`, control00-1f/7f, space, `~^:?*[\\`。Byte-sort/final LF; non-main exact58/SHA `4f3185217ec6f2309bab38a8fdbf25112b8f141204b139449686f993039899f2` at PLAN/preclaim/post-attempt/terminal。
  - s1_recovery_v4_executor_and_push: Executor launch exact argv `/usr/bin/env`,`-i`,`HOME=/Users/yusuke`,`PATH=/usr/bin:/bin`,`LC_ALL=C`,`LANG=C`,`/bin/bash`,`--noprofile`,`--norc`,`-c`,`<script>`, Popen env empty/cwd `/`/new verified PGID; thus BASH_ENV/ENV/SHELLOPTS/BASHOPTS/loader variables are absent before bash。Script exact ASCII/no LF-NUL is `exec /usr/bin/env -i ` plus each push-env assignment and git argv entry single-quoted/separated one space/no escape-prefix-suffix。Push env exact ordered: `HOME=/Users/yusuke`,`PATH=/usr/bin:/bin:/usr/sbin:/sbin:/opt/homebrew/bin`,`LC_ALL=C`,`LANG=C`,`GIT_CONFIG_NOSYSTEM=1`,`GIT_CONFIG_SYSTEM=/dev/null`,`GIT_CONFIG_GLOBAL=/dev/null`,`GIT_EXEC_PATH=/Library/Developer/CommandLineTools/usr/libexec/git-core`,`GIT_TERMINAL_PROMPT=0`,`GH_CONFIG_DIR=/Users/yusuke/.config/gh`。Git argv exact: `/usr/bin/git`,`--git-dir=/Users/yusuke/workspace/yrese/.git`,`--work-tree=/Users/yusuke/workspace/yrese`,`-c`,`credential.https://github.com.helper=`,`-c`,`credential.https://github.com.helper=!/opt/homebrew/bin/gh auth git-credential`,`-c`,`http.followRedirects=false`,`-c`,`http.extraHeader=`,`-c`,`push.followTags=false`,`-c`,`push.gpgSign=false`,`-c`,`push.useForceIfIncludes=false`,`push`,`--no-verify`,`--no-follow-tags`,`--recurse-submodules=no`,`--atomic`,`--force-with-lease=refs/heads/main:<base>`,`https://github.com/yusuketakuma/yrese.git`,`<commit>:refs/heads/main`。Hash framing: argv/executor/launcher every entry+terminal NUL, env ordered `key NUL value LF`, wrapper exact bytes。Push deadline120s, output caps1MiB, TERM2/KILL2/reap; fresh remote classification states `PUSH_APPLIED`,`ALREADY_PUSHED_RECOVERY`,`PUSH_NOT_APPLIED`,`REMOTE_DIVERGED`,`REMOTE_STATE_UNKNOWN` have the same exact main/refs/exit meanings stated here: only exit0+commit/same and nonzero-timeout-response-loss+commit/same proceed; base/same is not applied; other/missing/drift is diverged; unreadable invalid unknown; never retry。Origin tracking ref is non-authority。
  - s1_recovery_v4_ac9p: GH exact argv prefix `/opt/homebrew/bin/gh`,`api`,`--hostname`,`github.com`,`--method`,`GET`,`--header`,`Accept: application/vnd.github+json`,`--header`,`X-GitHub-Api-Version: 2022-11-28`; exact env `HOME=/Users/yusuke`,`PATH=/usr/bin:/bin:/usr/sbin:/sbin:/opt/homebrew/bin`,`LC_ALL=C`,`LANG=C`,`GH_CONFIG_DIR=/Users/yusuke/.config/gh`,`GH_HOST=github.com`,`GH_PROMPT_DISABLED=1`,`NO_COLOR=1`, cwd `/`, PGID15s/caps1MiB。Workflow endpoint suffix exact `/repos/yusuketakuma/yrese/actions/workflows/309812329/runs?event=push&branch=main&head_sha=<commit>&per_page=100&page=<n>`; top object requires nonbool int `total_count≥0` and list `workflow_runs`; every item requires positive nonbool int `id,workflow_id,run_attempt` and string `path,event,head_branch,head_sha,status`, plus `conclusion` null/string。Pages n=1 through empty next page, stable total/unique IDs/exact count。Match exact workflow_id309812329/path`.github/workflows/ci.yml`/push/main/head/attempt1。Before pin zero waits; first exactly-one pins even if already completed/success; transient statuses exact `requested,waiting,pending,queued,in_progress` require null and wait; completed+success proceeds; completed+other string fails; any other pair/multiple/disappearance after pin/ID drift STOP。Deadline1200s/interval15/max80 cycles。Jobs suffix `/repos/yusuketakuma/yrese/actions/runs/<run>/jobs?per_page=100&page=<n>`; top exact required total_count/jobs types, each item positive int id/string status+conclusion/list steps, each step positive int number/string name+status+conclusion; stable pagination/positive total/unique IDs/empty next; every job/step status completed and conclusion success, steps nonempty。Deployments suffix `/repos/yusuketakuma/yrese/deployments?sha=<commit>&per_page=100&page=1` must be exact empty list。Afterward fresh terminal all-ref requires main commit/non-main exact, then V4 local Git checks/clean state。Exact token `WP4158_RECOVERY_PUSH_PASS commit=<commit> base=<base> remote=https://github.com/yusuketakuma/yrese.git ref=refs/heads/main refs=4f3185217ec6f2309bab38a8fdbf25112b8f141204b139449686f993039899f2 run=<run> workflow=309812329 attempt=1 deployments=0`。Any argv/env/request/JSON/schema/pagination/time/parity drift STOP。
  - s1_recovery_v5_authority: Recovery push authority is complete V4 plus these V5 amendments; V1/V2/V3 remain non-authority。V5 replaces conflicting V4 file/launcher/request/state/claim/temporal/executor lifecycle/result details; unchanged V4 tool/config/context/local-Git/remote/ref/GH schema and terminal push token remain normative。
  - s1_recovery_v5_parent_and_files: For S1 or S2 runParent has two branches: absent => mkdir0700 then lstat/fstat/fsync parent; existing => require canonical no-symlink directory mode0700 uid501 gid20 and open no-follow/fstat; any other object STOP。Each slice uses fresh nonce/context binding。Leaf whitelist becomes `supervisor.py,approval.packet,approval.body,receipt.state,approved.state,claim.<attempt>.tmp,claim,executor.ready,spawn.state,executor.release,executor.evidence,result.state`。`approval.packet` contains only canonical request。After exact host reply root writes O_EXCL0600 `approval.body` containing the entire exact user UTF-8 body and O_EXCL0600 `receipt.state` containing complete canonical V4 receipt, fsyncing each file+dir; `approved.state` adds ordered final fields `approvalBodySha256,receiptSha256` after `monotonicDeadlineNs`。Supervisor parses body+receipt, rehashes, reconstructs exact token from request, and requires role/conversation/body/request equality。
  - s1_recovery_v5_request_launch_hashes: V4 request replaces `supervisorArgvSha256,executorArgvSha256` with ordered `supervisorLauncherArgvSha256,bootstrapLauncherArgvSha256,resumeLauncherArgvSha256,bashExecutorArgvSha256`; token uses `supervisor_launcher=<sha> bootstrap_launcher=<sha> resume_launcher=<sha> bash_executor=<sha>` and omits old keys。Supervisor launcher is complete V4 env-i/python recovery argv。Bootstrap launcher exact argv is `/usr/bin/env`,`-i`,`HOME=/Users/yusuke`,`PATH=/usr/bin:/bin`,`LC_ALL=C`,`LANG=C`,`/usr/bin/python3`,`-I`,`<supervisor.py>`,`--executor-bootstrap`,`<nonce-dir>`。Resume launcher is the same clean prefix with exact suffix `--classification-resume`,`<nonce-dir>`。Bash executor is exact V4 clean-env/bash argv。All hashes use entries+terminal NUL and are human-bound; Popen env empty/cwd `/`。
  - s1_recovery_v5_claim_alias: After successful hard link winner verifies temp and claim same regular inode/nlink2, unlinks its temp alias, fsyncs dir, requires claim nlink1 and no temp before spawn。Crash with claim+nlink2 permits classification-only resume to unlink only the unique verified same-inode temp then fsync/require nlink1; never spawn。Losers unlink only different-inode own temp。Extra temp/link/inode drift STOP。
  - s1_recovery_v5_spawn_fence: V5 claim temp/claim canonical fields append ordered `claimedMonotonicNs,bootstrapCutoffNs`, with cutoff=claimed+30000000000; this makes no-ready recovery bounded。Winner spawns pinned source in bootstrap mode with `start_new_session=True`, initially without mutation permission。Bootstrap O_EXCL-writes/fsyncs `executor.ready` fields `stateVersion,status,pid,pgid,startedMonotonicNs,releaseDeadlineNs` = `WP4158_EXECUTOR_READY_V5,WAITING,positive pid=pgid,start,claim bootstrapCutoff`, then polls only exact release and exits70/no mutation at cutoff if absent。Winner validates live identity/bytes, writes/fsyncs `spawn.state` fields `stateVersion,status,pid,pgid,readySha256,releaseDeadlineNs,mutationDeadlineNs` = `WP4158_SPAWN_STATE_V5,FENCED,same ids/hash,cutoff,cutoff+150000000000`, fsyncs dir, then O_EXCL-writes/fsyncs empty `executor.release`。Bootstrap revalidates claim/request/source/context/spawn/ready/release and both clocks, then execves exact bash executor in same pid/pgid。No durable spawn identity means no release/mutation。
  - s1_recovery_v5_quiescence: Winner supervises recorded PGID through mutationDeadline then TERM2/KILL2/reap。Loser/resume with claim never final-classifies while ready/spawn may identify a live group: before bootstrapCutoff/releaseDeadline wait; afterward verify exact pid/pgid, wait through mutationDeadline, TERM2/KILL2 if live, then poll `kill(pid,0)` and `getpgid` until ESRCH within5s。PID reuse/unverifiable identity => UNKNOWN STOP。Only after group quiescence, or claim with no ready/spawn/release after claim bootstrapCutoff, may fresh remote classification occur。Classification after claim ignores expired approval as mutation is forbidden, but still validates all immutable request/source/state hashes。Crash guards cover before/after ready, spawn fsync, release fsync, exec and response; no result precedes later authorized mutation。
  - s1_recovery_v5_results: Optional winner-only `executor.evidence` fields `stateVersion,attempt,pid,pgid,outcome,exit,stdoutSha256,stderrSha256,endedAtUtc`, V5 literal, are historical/non-reconstructible。`result.state` replaces V4 schema with remotely reproducible ordered fields `stateVersion,remoteClass,base,commit,remoteMain,nonTargetRefsSha256,observedAtUtc`; version=`WP4158_RESULT_STATE_V5`, class=`REMOTE_COMMIT|REMOTE_BASE|REMOTE_DIVERGED|REMOTE_UNKNOWN` from fresh post-quiescence snapshot。Existing result must parse and its base/commit/refs/class agree with fresh snapshot; observation time is not equality-compared。Mapping: remote commit + winner exit0=`PUSH_APPLIED`; remote commit with nonzero/timeout/response-loss or classification-only=`ALREADY_PUSHED_RECOVERY`; remote base=`PUSH_NOT_APPLIED`; other/missing=`REMOTE_DIVERGED`; unreadable/identity uncertainty=`REMOTE_STATE_UNKNOWN`。Only first two enter AC9P。
  - s1_recovery_v5_outer_and_subprocess: Root supervises supervisor PGID deadline1500s, output caps1MiB, TERM2/KILL2/reap。Success exit0/stderr empty/stdout exact terminal token LF; controlled failure exit70/stdout empty/one bounded `WP4158_RECOVERY_STOP reason=<fixed-code>\n` stderr。Timeout/cap/signal invokes same pinned source `--classification-resume <nonce-dir>`; fencing makes it classification-only and quiescence-first。Every local Git/remote/gh call requires exit0, stderr empty, no timeout/cap and complete stdout parse; partial output never counts。Preclaim failure has no mutation; postclaim failure quiesces/classifies; unreadable remote=>UNKNOWN, gh failure after landed remote preserves landing but AC9P STOPs。
  - s1_recovery_v6_authority: V4+V5 remain authority with these V6 amendments taking precedence only on release validation, executor ownership/quiescence, result sentinels and subprocess-output scope。
  - s1_recovery_v6_lock_identity: Add exact whitelist leaf `executor.lock` O_EXCL0600。Bootstrap opens it no-follow, verifies inode, takes nonblocking exclusive `flock`, clears FD_CLOEXEC, and keeps the descriptor across the exact bash/git/helper process tree; `executor.ready` and `spawn.state` append lock dev/ino。Controlled exact-tool guards prove an independent descriptor cannot acquire the lock during pre-release bootstrap, bash, git remote/helper and response phases, and can acquire it only after the last authorized descendant exits。If inheritance/lock behavior differs, implementation/landing STOPs before approval。Resume never signals based only on PID: busy verified lock proves an original authorized descendant still exists and therefore the recorded PGID has not become reusable; acquired lock proves no authorized descendant and forbids signaling any coincidentally reused PID/PGID。
  - s1_recovery_v6_cutoff_and_release: Immediately before writing `spawn.state` and again immediately before O_EXCL `executor.release`, winner requires strict `time.monotonic_ns() < bootstrapCutoffNs` and all V4/V5 clock predicates; equality/late means consumed/no release/no mutation。Bootstrap checks strict `now < releaseDeadlineNs` before polling, after observing release, and immediately before launching bash; equality/late unlinks nothing, exits70/no mutation。After release observation and before bash launch, bootstrap repeats exact V4 local Git outputs, tool/config/context/source, all four launcher/executor/wrapper/argv/env hashes, request/receipt/approved/claim/spawn/ready/release/lock identities and clocks; any drift exits70 without bash。Remote snapshot remains the immediately-preclaim authority and force lease prevents changed main。
  - s1_recovery_v6_quiescence: Ready-only/no-spawn cutoff is claim `bootstrapCutoffNs`; after it, resume checks lock。If acquired, authorized tree is quiescent and it never signals。If busy, exact recorded PGID must satisfy `killpg(pgid,0)=0` (EPERM or ESRCH is UNKNOWN); because busy lock prevents original group extinction/reuse, TERM2/KILL2 targets that group safely。Spawn state uses mutationDeadline; ready-only uses bootstrapCutoff. After deadline/termination, final quiescence requires both successful nonblocking lock acquisition and `killpg(pgid,0)` raising ESRCH; leader `kill(pid,0)` is non-authority。Any busy-lock/ESRCH, free-lock/live-group, EPERM, inode drift or failure to reach both conditions in5s => UNKNOWN STOP/no result token。Winner uses waitpid for its child plus the same group+lock final oracle。
  - s1_recovery_v6_results_and_output: Result grammar is total: `REMOTE_COMMIT` requires `remoteMain=<commit>` and refs exact SHA; `REMOTE_BASE` requires `<base>` and refs exact; `REMOTE_DIVERGED` requires remoteMain lowercase40hex or `MISSING` and nonTargetRefsSha256 lowercase64hex or `DRIFT`; `REMOTE_UNKNOWN` requires both fields literal `UNKNOWN`。Existing-result comparison uses these exact semantics。V5 strict exit0/stderr-empty/complete-parse subprocess rule applies only to local Git predicate, config, ls-remote and gh observation calls, not the bash executor push。Push stdout/stderr may be nonempty arbitrary bytes within separate1MiB caps and are hashed into optional executor evidence; exit/output never establishes remote state without fresh snapshot。
  - s1_recovery_v7_authority: V7 replaces V6 lock inheritance and all resume-side signaling; other V4+V5+V6 strict cutoff/revalidation/result/output clauses remain authority。Resume/loser never sends any signal to a recorded PID/PGID。
  - s1_recovery_v7_monitor: Add leaves `bash.ready,bash.release,mutation.state`。The first bootstrap remains a Python monitor process, holds `executor.lock` itself (FD_CLOEXEC retained; bash tree does not inherit), and never execs bash。After winner release and full V6 revalidation, monitor launches exact pinned source in `--bash-bootstrap <nonce-dir>` mode with start_new_session=True。Bash-bootstrap O_EXCL-writes/fsyncs `bash.ready` fields `stateVersion,status,pid,pgid,startedMonotonicNs,releaseDeadlineNs` = V7/WAITING/pid=pgid/start/start+30000000000 and waits only for bash.release, strict-before/after cutoff。Monitor validates live child and ready, O_EXCL-writes/fsyncs `mutation.state` fields `stateVersion,status,pid,pgid,readySha256,releaseDeadlineNs,mutationDeadlineNs` = V7/FENCED/ids/hash/cutoff/cutoff+150000000000, then strict-cutoff O_EXCL-writes/fsyncs empty `bash.release`。Bash-bootstrap repeats the complete V6 pre-launch predicate and only then execves exact bash executor in same pid/pgid。
  - s1_recovery_v7_parent_watchdog: Monitor is the direct parent of bash-bootstrap and keeps the child wait identity unreaped using `waitid(P_PID,pid,WEXITED|WNOHANG|WNOWAIT)` until the entire recorded group is extinct, so that numeric leader PID/PGID cannot be reused。It supervises through mutationDeadline; only this live parent holding that unreaped kernel child identity may TERM2/KILL2 the child PGID。After `killpg(pgid,0)=>ESRCH`, it reaps exact child, writes executor evidence, releases lock and exits。Exact guards cover leader exit with lingering helper, timeout, response loss and ensure no reap precedes group extinction。If monitor crashes, its lock releases but any bash group may continue; no other process signals it。
  - s1_recovery_v7_resume: Classification resume probes but never holds executor.lock beyond one nonblocking acquire/release and never interprets BUSY as signal authority。If monitor lock is BUSY, or `bash.ready/mutation.state` identifies a group for which `killpg(pgid,0)` is 0/EPERM, resume returns controlled `EXECUTOR_NOT_QUIESCENT` without `result.state` or remote terminal classification; concurrent resumes are harmless。Only when lock is acquirable and both monitor PGID and any recorded bash PGID return ESRCH in two samples one second apart may resume take fresh remote snapshot/write result。A crashed monitor plus stuck bash can therefore block recovery but can never cause unrelated signaling, premature terminal state or late mutation after terminal classification。Normal monitor path remains independently bounded; crash path is fail-closed。
  - s1_recovery_v7_launch_binding: V5 request/approval adds ordered `bashBootstrapLauncherArgvSha256` and token `bash_bootstrap_launcher=<sha>`。Exact launcher is the clean env-i/python prefix with suffix `--bash-bootstrap`,`<nonce-dir>`; entry+terminal-NUL hash is rechecked by winner, monitor and bash-bootstrap。Outer supervisor timeout never signals monitor/bootstrap PGIDs; it starts classification-resume, which observes the independently bounded monitor or returns nonterminal STOP。
  - s1_recovery_v8_authority: V8 replaces V7 unreaped-wait identity and augments descendant/remote settlement; resume no-signal and other V4-V7 clauses remain。
  - s1_recovery_v8_group_sentinel: Add leaves `sentinel.ready,monitor.lock,descendant.lock,result.state.v2` and launcher hash `sentinelLauncherArgvSha256`/token key `sentinel_launcher` for exact clean source mode `--group-sentinel <nonce-dir>`。Monitor holds monitor.lock。Using system-Python-supported `os.posix_spawn`, it creates sentinel source with `setpgroup=0`; sentinel O_EXCL-writes/fsyncs its pid=pgid and remains alive awaiting monitor termination。Monitor then creates bash-bootstrap with `setpgroup=<sentinelPid>`, making both direct children in the sentinel-pinned group; exact offline guards require these calls/pgids and system Python `os.posix_spawn` availability。Sentinel remains group leader until monitor cleanup, so PGID cannot be reused。
  - s1_recovery_v8_watchdog: Bash-bootstrap inherits non-CLOEXEC descendant.lock through bash/git/helper; sentinel does not。Monitor may reap bash child with system `waitpid` because live sentinel still pins PGID。At deadline or bash completion, monitor TERM2 then KILLs the sentinel-pinned group while sentinel is still an exact live direct child, then reaps both exact child PIDs with waitpid and requires killpg0 ESRCH plus successful descendant.lock acquisition before evidence/result。Only this monitor may signal。No `waitid`, WNOWAIT or libc wrapper is used。Guards require every exact executor descendant to remain in recorded PGID and retain descendant.lock; any setsid/setpgid/close escape or unexpected member fails before approval。If monitor crashes, resume never signals and terminal classification requires monitor.lock free, descendant.lock acquirable, and sentinel/bash PGIDs ESRCH in two samples; busy/escaped/live state is nonterminal STOP。
  - s1_recovery_v8_remote_settlement: `executor.release` or `bash.release` existence means a mutation attempt may have reached GitHub。After any such release, remote main=base is never terminal `PUSH_NOT_APPLIED` and never authorizes retry/S2, regardless of client exit or elapsed time; result class is exact `REMOTE_BASE_AMBIGUOUS` with base/refs and terminal outcome `REMOTE_STATE_UNKNOWN`。`REMOTE_BASE_UNATTEMPTED`/`PUSH_NOT_APPLIED` is allowed only when both release files are absent and all local monitor/sentinel/bash identities are quiescent。Remote commit proceeds as prior; other/unknown remain STOP。A delayed-server-commit guard returns base on initial polls then commit and requires no base result/token/retry before commit classification。This removes any claim that finite local quiescence proves receive-pack quiescence。
  - s1_recovery_v8_result_patch: V6/V7 result `remoteClass` enum becomes `REMOTE_COMMIT|REMOTE_BASE_UNATTEMPTED|REMOTE_BASE_AMBIGUOUS|REMOTE_DIVERGED|REMOTE_UNKNOWN`。Both base classes require remoteMain=base/refs exact; only UNATTEMPTED maps PUSH_NOT_APPLIED, AMBIGUOUS maps UNKNOWN。Existing result revalidation permits AMBIGUOUS→COMMIT monotonic upgrade by O_EXCL `result.state.v2` with the same schema/version suffix V8; no other replacement/downgrade is allowed。
  - s1_recovery_v9_authority: V9 replaces V8 spawn/lock/wait/result-settlement details; other V4-V8 clauses remain。
  - s1_recovery_v9_spawn_spec: Approval request adds ordered `spawnSpecSha256` and token `spawn_spec=<sha>`。Canonical spawn template uses ordered `name NUL value LF` records。Sentinel spec: executable `/usr/bin/env`; exact sentinel launcher argv; environment exact empty map; cwd `/`; file_actions empty; setpgroup `0`; setsigmask empty; setsigdef ordered `SIGINT,SIGTERM,SIGHUP,SIGALRM`; setsid absent/false。Bash-bootstrap spec: executable `/usr/bin/env`; exact bash-bootstrap launcher argv; environment empty; cwd `/`; file_actions exact `DUP2(descendantLockFd,9)` then `CLOSE(descendantLockFd)` when source fd≠9; setpgroup template `<validatedSentinelPid>`; same mask/defaults; setsid absent。Template SHA is human-bound; runtime substitutes only positive sentinel pid and validates actual pgid。Both call exact `/usr/bin/python3` `os.posix_spawn` with these keyword values; availability/setpgroup/FD9/signal behavior guards run under pinned system Python。
  - s1_recovery_v9_lock_lifecycle: V9 leaf whitelist removes historical `executor.lock` and keeps distinct `monitor.lock,descendant.lock`。Monitor O_EXCL-opens/flocks monitor.lock for its lifetime。After sentinel ready, monitor O_EXCL-opens descendant.lock0600, exclusive-flocks it, clears source CLOEXEC, and posix_spawns bash-bootstrap with exact FD9 DUP2 spec。Bash-bootstrap fstat-verifies FD9 against descendant.lock, confirms an independent open cannot flock, makes FD9 inheritable across bash exec, and records dev/ino/fd=9 in bash.ready/mutation.state。After ready validation and before bash.release, monitor closes its descendant-lock source FD and independently confirms lock remains busy; thereafter only bash-bootstrap/bash/git/helper open descriptions hold it。On exact executor completion the lock becomes acquirable; any close/escape guard failure STOPs before approval or terminal evidence。Sentinel never receives descendant lock。
  - s1_recovery_v9_wait_state: Monitor tracks booleans `bash_reaped=false,sentinel_reaped=false` and stored statuses。Polling `waitpid(bashPid,WNOHANG)` returns0 while live or returns exact pid once, atomically stores status and sets bash_reaped; it is never called again for bash after true。Sentinel pins PGID throughout。At bash completion or deadline monitor TERM2/KILLs the pinned group; if bash_reaped=false it waitpid-reaps bash exactly once, then waitpid-reaps sentinel exactly once; if already true it reaps sentinel only。Then require killpg0 ESRCH and descendant lock acquirable。No child is double-waited and no non-reaping API is used。Exact normal/timeout/leader-first/helper-linger guards assert call counts/status preservation。
  - s1_recovery_v9_result_state: Immediate post-release base persistence is explicitly allowed and required as `result.state` with exact ordered fields `stateVersion,remoteClass,base,commit,remoteMain,nonTargetRefsSha256,observedAtUtc`, version `WP4158_RESULT_STATE_V9_BASE`, class `REMOTE_BASE_AMBIGUOUS`, base/commit/base/exact refs/time。It is nonterminal: no PASS token, PUSH_NOT_APPLIED, retry or S2。Delayed-server guard may observe any number of such base snapshots and forbids only UNATTEMPTED/not-applied/retry/terminal claims。If a later fresh snapshot is commit, exactly one O_EXCL `result.state.v2` may be written with ordered fields `stateVersion,priorResultSha256,remoteClass,base,commit,remoteMain,nonTargetRefsSha256,observedAtUtc`, version `WP4158_RESULT_STATE_V9_COMMIT_UPGRADE`, prior exact SHA, class REMOTE_COMMIT, base/commit/commit/exact refs/time。When both exist, v2 is authoritative only if v1 parses as exact ambiguous, prior hash matches, fresh remote remains commit; any other pair/coexistence/change STOPs。Without v2, v1 remains ambiguous forever and resume may only re-observe, never mutate。
  - s1_recovery_v10_authority: V10 fully replaces all conflicting V5/V7/V8/V9 request-order, launcher/spawn and result clauses; lifecycle/safety behavior not conflicting remains。
  - s1_recovery_v10_request_order: Canonical request version is exact `WP4158_FORCE_LEASE_APPROVAL_V10`; exact final ordered fields are `approvalVersion,nonce,issuedAtUtc,expiresAtUtc,monotonicIssuedNs,monotonicDeadlineNs,base,commit,remote,ref,toolManifestSha256,localConfigSha256,executionContextSha256,supervisorSourceSha256,supervisorLauncherArgvSha256,bootstrapLauncherArgvSha256,resumeLauncherArgvSha256,bashBootstrapLauncherArgvSha256,sentinelLauncherArgvSha256,bashExecutorArgvSha256,spawnSpecSha256,wrapperSha256,argvSha256,envSha256,nonTargetRefsSha256`。No later insertion rule applies。Exact whole user body is `APPROVE_WP4158_FORCE_LEASE_V10 request=<requestSha> nonce=<nonce> base=<base> commit=<commit> refs=<refsSha> tools=<toolSha> config=<configSha> context=<contextSha> supervisor=<sourceSha> supervisor_launcher=<sha> bootstrap_launcher=<sha> resume_launcher=<sha> bash_bootstrap_launcher=<sha> sentinel_launcher=<sha> bash_executor=<sha> spawn_spec=<sha> wrapper=<sha> argv=<sha> env=<sha> issued=<utc> expires=<utc> mono_issued=<ns> mono_deadline=<ns>` with one ASCII space between fields and no terminal LF。Request/body hash preimages are unique。
  - s1_recovery_v10_spawn: Both `os.posix_spawn` calls inherit the monitor's verified cwd `/`; cwd is not a keyword/file_action and each child first requires `os.getcwd()=="/"`。Exact supported call shape is `os.posix_spawn(path,argv,env,file_actions=...,setpgroup=...,resetids=False,setsid=False,setsigmask=(),setsigdef=(SIGINT,SIGTERM,SIGHUP,SIGALRM))`, scheduler omitted。Sentinel path=`/usr/bin/env`, argv=exact sentinel launcher, env=`{}`, file_actions=(), setpgroup=0。Before bash spawn monitor proves fd8 EBADF, O_EXCL-opens/flocks descendant.lock, normalizes the same open description to inheritable source fd8 with dup2 if needed and closes the raw alias; exact bash file_actions are `(POSIX_SPAWN_DUP2,8,9),(POSIX_SPAWN_CLOSE,8)`, path env, argv exact bash-bootstrap launcher, env empty, setpgroup exact validated sentinelPid。Only runtime substitution is sentinelPid; source fd is fixed8。After bash ready/FD9 identity, monitor closes fd8。V10 explicitly replaces V7 `start_new_session=True` for bash-bootstrap and all earlier spawn templates。
  - s1_recovery_v10_result_totality: First post-quiescence snapshot always writes exact `result.state` ordered fields `stateVersion,remoteClass,base,commit,remoteMain,nonTargetRefsSha256,observedAtUtc`, version `WP4158_RESULT_STATE_V10_INITIAL`。Allowed class/field pairs: `REMOTE_COMMIT`=commit/exact refs; `REMOTE_BASE_AMBIGUOUS`=base/exact refs when any release exists; `REMOTE_BASE_UNATTEMPTED`=base/exact refs only when no release; `REMOTE_DIVERGED`=lowercase40hex-or-MISSING and lowercase64hex-or-DRIFT; `REMOTE_UNKNOWN`=UNKNOWN/UNKNOWN。COMMIT enters AC9P; BASE_UNATTEMPTED maps not-applied; BASE_AMBIGUOUS/DIVERGED/UNKNOWN STOP with no retry/S2。Only initial BASE_AMBIGUOUS may later gain O_EXCL `result.state.v2` ordered fields `stateVersion,priorResultSha256,remoteClass,base,commit,remoteMain,nonTargetRefsSha256,observedAtUtc`, version `WP4158_RESULT_STATE_V10_COMMIT_UPGRADE`, prior exact SHA, class COMMIT and commit/exact refs。v2 is authoritative only with matching ambiguous v1 and fresh commit; all other coexistence STOP。Initial COMMIT/DIVERGED/UNKNOWN never fabricate base/v2。
  - s1_recovery_v11_authority: V11 finalizes V10 spawn manifest bytes and result observational upgrades; other V4-V10 authority remains。
  - s1_recovery_v11_spawn_manifest: `spawnSpecSha256` is SHA-256 of exactly 23 ordered `name NUL value LF` records, no sorting/extras: `schema=WP4158_POSIX_SPAWN_SPEC_V11`; then for prefix `sentinel` exact names `.path,.argvSha256,.envSha256,.cwdInherited,.fileActions,.setpgroup,.resetids,.setsid,.setsigmask,.setsigdef,.scheduler`; then the same 11 names for prefix `bash`。Sentinel values are `/usr/bin/env`, sentinel launcher argv SHA, empty-env SHA `e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`, `/`, `EMPTY`, `0`, `false`, `false`, `EMPTY`, `2,15,1,14`, `OMITTED`。Bash values are `/usr/bin/env`, bash-bootstrap launcher argv SHA, same empty-env SHA, `/`, `DUP2:8:9,CLOSE:8`, literal template `<sentinelPid>`, `false`, `false`, `EMPTY`, `2,15,1,14`, `OMITTED`。All values are exact ASCII shown; final LF required。Runtime manifest substitutes only positive validated decimal sentinelPid for the literal template and must otherwise byte-match/rederive every call argument; template SHA is the approval field/token value。
  - s1_recovery_v11_result_upgrade: `result.state.v2` COMMIT upgrade is permitted iff initial `result.state` class is exactly `REMOTE_BASE_AMBIGUOUS` or `REMOTE_UNKNOWN`, never DIVERGED/UNATTEMPTED/already COMMIT。The V10 v2 ordered schema/version/priorResultSha rules apply; fresh snapshot must be exact commit+refs, upgrade is O_EXCL and observational only, and no mutation retry occurs。When v2 exists it is authoritative only after prior file hash/class and fresh commit revalidation; concurrent identical creators accept one exact winner, differing bytes STOP。Thus transient unreadable observation can recover to AC9P while ambiguous base and UNKNOWN can never authorize another push。
  - s1_recovery_v12_authority: V12 replaces sentinel posix_spawn/request/spawn-manifest portions with a fork-release witness; bash posix_spawn and other V4-V11 authority remain。
  - s1_recovery_v12_sentinel_intent: Add exact whitelist leaves `sentinel.intent,sentinel.exit`。Single-threaded monitor first requires its signal mask empty, then blocks exact SIGINT/SIGTERM/SIGHUP/SIGALRM before any target-FD check, clock capture or pipe operation。While blocked it captures one absolute `sentinelReleaseDeadlineNs=time.monotonic_ns()+30000000000`, then requires target FDs5,6,7,8,10 EBADF, creates two raw pipes, duplicates the four raw descriptions with `fcntl(F_DUPFD_CLOEXEC,20)` to distinct backups≥20, closes raw aliases, dup2s backups to release5/6 and guardian7/10, closes backups, verifies identities/directions and fd8 EBADF。Any collision/drift STOPs without clobber。Parent write ends CLOEXEC。Still blocked, it calls `os.fork()`; the child receives only the absolute deadline and pipe ends, never challenge bytes。Child immediately verifies all four blocked before closing parent ends and waiting for one-byte release; EOF/wrong/deadline exits70。Parent closes child ends, then in an exception-safe finally restores its exact original empty mask once; restoration failure closes parent writes so blocked child receives EOF, invokes the exact pre-ready settlement below, and STOPs。Only after fork and mask restoration, parent obtains exact32 bytes from `os.urandom`, retains them only in parent memory and computes `guardianChallengeSha256`; entropy short-read/error closes both writes and settles child。Parent then fsyncs `sentinel.intent` ordered fields `stateVersion,status,pid,parentPid,releaseDeadlineNs,releaseReadFd,guardianReadFd,guardianChallengeSha256` = `WP4158_SENTINEL_INTENT_V12,FENCED,child,self,<same inherited absolute deadline>,5,7,<SHA-256 of exact challenge bytes>`。It complete-writes the exact32 raw challenge bytes to fd10 strictly before the same deadline, writes `R` to fd6 and closes fd6; any write/error/deadline closes both writes and settles child without ready。Crash before release causes EOF; no unrecorded released child exists。Guards inject each signal during target normalization, immediately post-fork, release wait, parent restoration and parent-only entropy/write, requiring no child handler/FD leak/clobber and exact parent mask recovery。
  - s1_recovery_v12_sentinel_child: Child remains four-signal-blocked from inherited pre-fork mask through FD closing, release wait, setpgid and exec。After exact release strictly before its inherited absolute deadline it closes fd5, marks only guardian7 inheritable, re-verifies the blocked set, sets/verifies all four `SIG_DFL` while blocked, verifies identity and execves sentinel with exact empty environment without unmasking。Python 3.9 startup may install `signal.default_int_handler` for SIGINT, but all four signals remain blocked throughout startup。At the first sentinel-source action, before filesystem/pipe work, it requires the inherited blocked set, sets/verifies SIGINT/SIGTERM/SIGHUP/SIGALRM all `SIG_DFL` while blocked, then performs the sole `pthread_sigmask(SIG_SETMASK,())` and verifies empty mask/defaults。Pending one-of-four delivery at that unmask must terminate the non-mutating sentinel before ready; guards cover all four, Python-startup SIGINT, and forbid any inherited/default-int handler execution。Only after this transition sentinel re-verifies cwd, exact pid=pgid and `sentinel.intent`, then complete-reads exactly32 challenge bytes from guardian fd7 strictly before the intent deadline and requires their SHA to equal `guardianChallengeSha256`; EOF, extra/short data, timeout or substitution STOPs before ready。It then O_EXCL-writes/fsyncs `sentinel.ready` in exact ordered fields `stateVersion,status,pid,pgid,parentPid,intentSha256,guardianFd,guardianChallengeSha256,releaseDeadlineNs` = `WP4158_SENTINEL_READY_V12,READY,<self>,<self>,<intent parentPid>,<SHA-256 of exact intent bytes>,7,<validated challenge SHA>,<same absolute deadline>`。Before bash spawn the monitor validates all fields/hash/identities and keeps guardian write fd10 open。After challenge consumption sentinel polls only guardian fd7 while alive: EOF, any later byte, error or parent identity drift exits70; monitor cleanup closes fd10, and monitor crash closes it automatically。Exact substitution guards replace fd7 or fd10 independently and require no ready。
  - s1_recovery_v12_pre_ready_wait: The monitor is the sole parent and owns exactly-once settlement from fork return until validated `sentinel.ready`。It tracks `sentinel_reaped=false` and stored wait status; every parent failure, release EOF/wrong/deadline, pending-signal death, pre-ready setup/exec failure or ready validation failure first closes fd6/fd10, then polls `waitpid(childPid,WNOHANG)` for at most2s。Each poll returning0 leaves state unchanged; the sole poll returning exact child pid atomically stores status and flips `sentinel_reaped=true`, and waitpid is never called again for that child。If still unreaped, the direct-child kernel identity authorizes PID-directed TERM2 then KILL2, each followed by the same bounded poll rule; bash has not been spawned in this phase。Failure to reap after the post-KILL2 deadline exits controlled `SENTINEL_REAP_TIMEOUT` with no result/continuation claim and lets process exit close guardian; no blocking wait is allowed。After a stored status and before STOP, the monitor best-effort O_EXCL-writes/fsyncs `sentinel.exit` ordered fields `stateVersion,status,pid,waitStatus,endedMonotonicNs` = `WP4158_SENTINEL_EXIT_V12,PRE_READY_EXIT,<child>,<stored decimal wait status>,<decimal monotonic ns>`; record failure cannot authorize continuation/result。Exact guards cover natural70, all four pending-signal deaths, TERM, KILL, exec-before-ready, write/record failures, reap-timeout, stored-status preservation and waitpid count=1。After ready validation, V9 sentinel wait state takes over with the same stored boolean/status and still forbids double wait。
  - s1_recovery_v12_wait_totality: This clause replaces V9 waitpid mechanics for both direct children while preserving V9 cleanup order。Every child has one boolean reaped flag and optional stored status; the only wait operation is `waitpid(exactPid,WNOHANG)` under an absolute monotonic deadline。Return0 leaves state unchanged; return exact pid atomically stores status/sets its flag; any other return, `ChildProcessError`, `ECHILD` or non-EINTR error is identity-unknown STOP with no result。EINTR alone retries while strictly before the unchanged deadline; this wait rule imports no verifier-oracle deferred-signal state, and a signal disposition that terminates or raises simply leaves the wait call without a successful transition。No wait call occurs after a true flag and no blocking wait exists。Pre-ready uses the V12 PID TERM/KILL deadlines above。Post-ready, bash alone may be polled/reaped during execution。Cleanup TERM2/KILL2-signals the group while the exact sentinel direct child remains unreaped; then any still-false bash flag and finally sentinel flag must reach exact-pid return within5s。Only after both exact statuses are stored does monitor require descendant lock acquirable and `killpg(pgid,0)` ESRCH; no signal is sent after sentinel reap, and a reused/live/EPERM PGID causes safe nonterminal STOP。Timeout/error is nonterminal STOP and monitor exit closes guardian。Exact guards inject EINTR and every error/unexpected return at natural completion, TERM, KILL, leader-first/helper-linger and sentinel-last boundaries, asserting bounded duration, preserved status, zero double-wait and no result before both flags plus post-reap group/lock oracle。
  - s1_recovery_v12_boot_epoch: Pinned system Python loads exact dyld image name `/usr/lib/libSystem.B.dylib` via `ctypes.CDLL(path,use_errno=True)` and declares `sysctlbyname.argtypes=(c_char_p,c_void_p,POINTER(c_size_t),c_void_p,c_size_t)`, `restype=c_int`。Exact `DarwinTimeval` fields are signed `tv_sec:c_long` and signed `tv_usec:c_int`; guards require `sizeof(c_long)=8,sizeof(c_int)=4,sizeof(DarwinTimeval)=16`, field offsets0/8, raw padding bytes12..15 zero, returned length16, result0 after `set_errno(0)`, errno0, seconds positive and microseconds0..999999。Two separately zero-initialized calls with name bytes `b"kern.boottime"` and null new-value pointers must return identical field/raw bytes。For `b"kern.bootsessionuuid"`, each of two samples first calls with null old-value and requires returned length37, then reads into a separately zeroed37-byte buffer and requires result0/errno0/returned37, exactly one terminal NUL, no internal NUL, and 36 uppercase ASCII bytes matching `[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}`; both samples must be byte-identical。Any library/prototype/type/signedness/offset/size/length/padding/syscall/range/sample/UUID drift STOPs。Seconds encoding is the unique shortest unsigned decimal ASCII matching `[1-9][0-9]*` with no sign or leading zero; microseconds encoding is exactly six decimal ASCII digits matching `[0-9]{6}` including leading zeros。`bootSessionSha256` is SHA-256 of the sole preimage `b"kern.boottime\x00"+seconds+b"."+microseconds+b"\n"+b"kern.bootsessionuuid\x00"+uuid36+b"\n"`。Controlled mutants include c_long/c_long, unsigned/swapped fields, wrong offsets/packing/length/prototype/library/padding, seconds `+`/leading-zero/nonminimal forms, microsecond non-six-digit forms, UUID case/shape/length/NUL changes, same-boottime/different-UUID, and missing/extra/reordered NUL/dot/LF; all must fail before request creation。The hash is captured into the approval request and rederived before approval acceptance, claim, fork, each release and every resume。Mismatch invalidates approval and all mutation/signal authority permanently for that nonce; it never waits on persisted monotonic deadlines or inspects recorded PID/PGID as identity。Classification-only resume treats an absent monitor/descendant lock leaf as free, but for each existing leaf requires no-follow identity validation plus successful nonblocking acquire/release; any durable state that records a lock identity must match an existing leaf, and invalid/busy/drifted locks STOP。After proving no current writer it takes a fresh remote snapshot and applies V10 total result semantics: any release file means BASE is AMBIGUOUS, no release means BASE_UNATTEMPTED, COMMIT may proceed, other/unreadable STOP。No retry/S2 occurs except after COMMIT+AC9P。Exact rollback/reboot guards cover every crash cut with changed boot hash and numerically smaller, equal and larger current monotonic values, lock absent/free/busy/drift, and require bounded no-signal/no-mutation classification; same-boot backward monotonic remains UNKNOWN STOP。
  - s1_recovery_v12_git_object_identity: This clause globally amends every Git invocation used by any root/reviewer/supervisor step from recovery PLAN construction through IMPLEMENTATION/BUG_REFACTOR/VALIDATION/COMMIT/PUSH and S2 recovery, including base/candidate/non-ledger parity, disposable-index read-tree/add/write-tree, diff/status/show/rev-list/diff-tree/cat-file, AC9C/AC9L packets, config/remote reads and bash push。Every argv uses absolute `/usr/bin/git` and inserts global option `--no-replace-objects` immediately after it。Every child uses exact minimal V4 Git env with `GIT_NO_REPLACE_OBJECTS=1` immediately before `GIT_TERMINAL_PROMPT=0`; disposable candidate operations may additionally set only one repository-external absolute `GIT_INDEX_FILE` whose empty/new identity and removal/residue are gate evidence。Before and after each material gate, the same no-replace prefix/env runs exact suffix `for-each-ref,--format=%(refname)%00%(objectname),refs/replace` and requires empty stdout/stderr/exit0, while no-follow lstat requires `.git/info/grafts` absent; loose, packed or namespaced replacement/graft ambiguity STOPs。The final preclaim/pre-release cycles repeat both guards with no external command before one-shot mutation; push uses the same flag/env even though pack transfer normally ignores replacements。Each fresh reviewer packet includes exact no-replace command/env grammar plus empty-replace/absent-graft observations; candidate tree, AC9C/AC9L commit/tree/parent/message/exact4 evidence and final human request hashes are derived only under this grammar。Updated launcher/wrapper/argv/env hashes remain request- and human-bound。Controlled loose/packed replace refs, legacy grafts, PLAN/candidate/AC9C/AC9L/S2 flag or env removal, reordered env and replacement-commit parent/tree/message/exact4 mutants must fail before gate approval or spawn。
  - s1_recovery_v12_raw_commit: V12 replaces every authoritative `rev-list --parents` ancestry result and any show-derived parent/tree/message assertion with a graft-independent raw-object oracle。Under the global no-replace grammar, exact `cat-file,commit,<oid>` must return exit0/stderr empty, complete stdout bytes within1MiB and no timeout。Root independently requires SHA-1 of `b"commit "+shortest_decimal_length+b"\x00"+raw` equals lowercase `<oid>` and records SHA-256(raw)。Raw bytes require LF termination, no NUL/CR, first header exact `tree <lowercase40>\n`, then the exact number/order of `parent <lowercase40>\n` lines expected for that already-existing commit, later exact single `author ...\n` and `committer ...\n`, no continuation/encoding/gpgsig/extra header, one blank LF, and exact body bytes。`cat-file -t` must be `commit\n`, and every parsed tree independently has type `tree\n`。Phase split is mandatory: PLAN/IMPLEMENTATION/BUG_REFACTOR/VALIDATION/AC9C occur before recovery commit creation and bind `baseCommit,baseCommitObjectSha256` plus the base object's own parsed tree/parents/message, while separately binding `candidateTree,candidateMessage`; they forbid any future `EXPECTED_COMMIT`/`commitObjectSha256` claim。After root creates the recovery commit, AC9L/PUSH/preclaim/pre-release/terminal bind `commit,commitObjectSha256` and require its parsed tree=`candidateTree`, sole parent=`baseCommit`, body=`candidateMessage+"\n"`。S2 PLAN binds the already-landed S1 commit/raw SHA as its base and separately binds the S2 candidate tree/message。Final human request/body occur postcommit and bind only the recovery `commitObjectSha256`/`commit_object`。Preclaim, pre-release and terminal rederive all bytes/hash/parse。Transient graft insert-use-remove, static graft, replacement ref, raw length/hash/header/body mutation, phase-confused future commit and rev-list/raw disagreement must fail; rev-list may be diagnostic only and can never authorize review, commit or push。
  - s1_recovery_v12_resume_gap: Resume reads intent first。Before readiness deadline, missing/partial ready is nonterminal wait。After deadline, if ready valid it applies existing monitor/sentinel/bash quiescence rules; if ready missing/partial it requires recorded intent PID `kill(pid,0)=>ESRCH` in two samples one second apart before any result, otherwise controlled `SENTINEL_NOT_QUIESCENT` with no signal/result。PID reuse can only cause safe permanent STOP because resume never signals。Intent absent means release never occurred; after monitor lock free/bootstrap cutoff and no sentinel/bashes it may classify。Exact crash guards cover fork return before intent, intent fsync before release, release before exec, exec before ready, and parent death。
  - s1_recovery_v12_request_spawn_patch: Final request exact field order is `approvalVersion,nonce,issuedAtUtc,expiresAtUtc,monotonicIssuedNs,monotonicDeadlineNs,base,commit,commitObjectSha256,remote,ref,toolManifestSha256,localConfigSha256,executionContextSha256,bootSessionSha256,supervisorSourceSha256,supervisorLauncherArgvSha256,bootstrapLauncherArgvSha256,resumeLauncherArgvSha256,bashBootstrapLauncherArgvSha256,sentinelLauncherArgvSha256,sentinelForkSpecSha256,bashExecutorArgvSha256,spawnSpecSha256,wrapperSha256,argvSha256,envSha256,nonTargetRefsSha256`。Final whole-body key order is `request nonce base commit commit_object refs tools config context boot_session supervisor supervisor_launcher bootstrap_launcher resume_launcher bash_bootstrap_launcher sentinel_launcher sentinel_fork_spec bash_executor spawn_spec wrapper argv env issued expires mono_issued mono_deadline`; no insertion rules remain。Sentinel fork manifest exact ordered `name NUL value LF`: `schema=WP4158_SENTINEL_FORK_SPEC_V12`,`runtime=/usr/bin/python3@3.9.6`,`threading=single`,`parentPreForkMask=BLOCK-2-15-1-14`,`parentPostForkMask=EMPTY`,`fdNormalization=targets-empty,raw-to-backup-ge20,close-raw,backup-to-5-6-7-10,close-backup`,`releaseFds=5,6`,`guardianFds=7,10`,`reservedFd=8`,`releaseByte=R`,`guardianChallengeBytes=32`,`guardianChallengeOrigin=parent-post-fork`,`deadlineNs=30000000000`,`deadlineOrigin=parent-pre-fork-monotonic`,`childSignalTransition=inherited-blocked,set-defaults,exec-blocked,sentinel-set-defaults,setmask-empty`,`signalDefaults=2,15,1,14`,`childAction=close-parent-fds,read-release,set-defaults,setpgid,exec-blocked`,`parentAction=close-child-fds,restore-mask,generate-challenge,fsync-intent,write-guardian-challenge,write-release,hold-guardian`; final LF。V11 spawn manifest sentinel records are superseded by `sentinel.mode=FORK_RELEASE` and `sentinel.forkSpecSha256=<sha>`; bash11 remain, exact14 records。Only bash uses os.posix_spawn。
  - s1_recovery_v13_context_drift_candidate: 2026-07-26のroot read-only再観測で、worktree/gitdir/config/codexParentのmode/uid/gid/inoはV4固定値と一致した一方、各`lstat/fstat`の`st_dev`はV4の`16777233`ではなくexact `16777228`で一致した。`/Users/yusuke/.codex/run`は不存在で、作成していない。V4はfixed identity driftをSTOP条件とするため、現V12 authorityのままcontext-bound request/approval/claim/pushへ進むことを禁止する。本項は観測されたdriftと改版候補を記録するだけでauthorityではない。V13 PLANは4 objectのdevだけを`16777228`へ置換し、inode/mode/uid/gid、runParent safe-create/capture、boot-session/no-replace/raw-commit/fork-release/one-shot/human gateを不変とする最小amendmentとしてfresh independent PLAN reviewを必要とする。review PASSまではoffline/pure supervisor実装とread-only再観測だけを許可し、runParent作成、approval sealing、claim、signal authority、push、AC9P、S2を禁止する。
  - s1_recovery_v13_offline_preflight: authority/sourceは変更せず、V12 source `2a759801…`の4 context rowとそのstatic guardにあるexact5 literal `16777233`だけをメモリ上で`16777228`へ置換した候補はSHA-256 `0430d462f7a435d028c63720fb6ce61bd6524284e9fdc59673223b2eed68e187` / 10934 LF lines。Repo外private temp0600へ一時materializeし、workspace cwd・sanitized envで`/usr/bin/python3` 3.9.6と`/opt/homebrew/bin/python3` 3.14.6のexact recovery self-testを各PASS（positives2/negatives95/cleanup10/signal40/24/54/mutations11/residue0）、temp residue0。さらに1秒間隔3 sampleで4 objectのno-follow `lstat/fstat` identityが全てdev=`16777228`かつ既存ino/mode/uid/gidと一致し、runParent absentを維持した。これはV13最小amendmentのoffline feasibilityだけを示し、PLAN/IMPLEMENTATION independent PASS、live authority、runParent作成、approval/claim/signal/push/AC9Pを一切付与しない。
  - s2_recovery_plan_oracle: After S1 recovery AC9P only, external packet requires HEAD/pinned main=`LANDED_S1_RECOVERY_COMMIT`, exact push CI terminal token, tracked/index clean, extracted `S1_RECOVERY_VERIFIER_SHA`/LF count, and disposable S2 `EXPECTED_CANDIDATE_TREE` with exact4 landing-record-only diff。Verifier bytes, rights/handoff data and nonclaims are byte-identical; no verifier/network execution in PLAN。Exact token=`WP4158_S2_RECOVERY_PLAN_READY base=$LANDED_S1_RECOVERY_COMMIT tree=$EXPECTED_CANDIDATE_TREE paths=4 scope=landing-only verifier=$S1_RECOVERY_VERIFIER_SHA ci_run=$S1_RECOVERY_CI_RUN execution=forbidden`。Then S2 recovery uses fresh six gates and the same human-gated recovery landing state machine with S2-specific base/commit。
  - slice_validation_matrix: Original S0/S1/S2はhistorical PLAN_READY/direct/AC9 branch-PR matrixを使用した。S1 recovery/S2 recoveryはrecovery専用PLAN/interpreter/signal/landing oraclesだけを使用し、historical branch/PR/fixed-main AC9を併用しない。Recovery IMPLEMENTATION onward adds verifier syntax、both-interpreter exact self-test/live oracles; VALIDATION adds full workspace gates; COMMIT uses common AC9C/AC9L content binding; PUSH uses only recovery main-push AC9P。Any further recovery repair uses its own PLAN_READY、finding-focused exact4 diff、repaired-slice direct commands、recovery landing oracle。Every material gate fresh exactly5。UI/browser/accessibility N/A。
  - review/landing: Original S0/S1/S2 terminal historyとrecovery chronologyを混同しない。S1 recovery/S2 recoveryはそれぞれPLAN→IMPLEMENTATION→BUG_REFACTOR→VALIDATION→COMMIT→PUSHの各material gateにfresh exactly5 read-only reviewers、全員PASSのみ進行。Recovery push後はlocal HEAD/pinned remote main parity、deployments0、exact push-event head attempt1 CI successを確認し、origin tracking-ref parity、PR parity、fixed old main guardを要求しない。S2 recovery tracked recordはLANDED_S1_RECOVERY_COMMITの事実だけを記録し、S2自身のfuture commit/push/CI/parityを先取りしない。
  - human_gates: legal/license最終判断、FHIR/conformance、clinical/claim、patient safety、product/architecture、unsigned supply-chain risk acceptanceはtechnical reviewで解除しない。rights matrixはrightsholder、authoritative terms URL/date/version、validation cache/runtime terminology service/UI display/export/public IG/test bundle/Partner Sandbox/SDK/Bulk Dataごとのpermitted/attribution/redistribution/derivative/update obligations、decision、human authority、decision dateを持ち、未決は`UNRESOLVED`。human authorityの記録前はpackage lock/runtime/adoption/publication/SSOT変更をSTOP。加えてrecovery landingのexact `--force-with-lease` commandは、commit作成後かつ実行直前にbase/commit/URL/ref/command hashへboundした明示human approvalなしには実行禁止。Approval未取得はimplementation/validation/commit reviewを無効化しないが、PUSH mutationをfail-closedで停止する。
  ...
  - s1_bug_refactor_history_31: thirty-first projected source `9661c715c218aa8c7ebcf4fdabb42125455fa6d2f30a9d08619b435a669d2e87` / 2544 LF linesはlive marker extraction `e096d18ce95dbd0f1167e12c918747393f23ceafdb48cc760e462d2d2c0bf151` / 2610 LF linesと不一致で、20/20 self-test greenがunbound bytesを検出しないfalse-greenだった。Fresh reviewers1〜3は全員FAIL。reviewer1/2はunmask→actual handler restore→deactivateの順序でrestore直後signalがSIGTERM=-15、SIGHUP=-1、SIGALRM=-14、SIGINT primary secondary-note化となるraceを再現。reviewer1/3は`f4d0f8f…`がS0 childから10 pathsを変更してexact4/package/lock/runtime/code/toolchain nonclaimを破ることもblockした。Recorded live PASSはprojected sourceにもactual e096 sourceにもbinding不能でnon-evidence。
  - s1_recovery_plan_candidate: recovery base=`f4d0f8f…`、scope=exact4、inline sourceは既知race持ち非authority。Thirty-third `837ea4bb9e094cdf80da2abc892c37fd2ce83528`はreviewer1/2 FAILでprecommit future-rawをblock。Thirty-fourth candidate tree `ef291d965197c12bc9906e73bbac56c68b2e21a2`はfresh reviewer1〜5が5/5 PASS。Exact token=`WP4158_S1_RECOVERY_PLAN_READY base=f4d0f8f71a62c3e55a57a79ad57092b10620cc70 tree=ef291d965197c12bc9906e73bbac56c68b2e21a2 paths=4 source=e096d18ce95dbd0f1167e12c918747393f23ceafdb48cc760e462d2d2c0bf151 python_system=/usr/bin/python3@3.9.6 sys_system=/Library/Developer/CommandLineTools/usr/bin/python3 runtime_system=/Library/Developer/CommandLineTools/Library/Frameworks/Python3.framework/Versions/3.9/bin/python3.9 python_homebrew=/opt/homebrew/bin/python3@3.14.6 sys_homebrew=/opt/homebrew/opt/python@3.14/bin/python3.14 runtime_homebrew=/opt/homebrew/Cellar/python@3.14/3.14.6/Frameworks/Python.framework/Versions/3.14/bin/python3.14 signal_protected=40 signal_boundary=24 signal_combinations=54 mutations=11 remote=https://github.com/yusuketakuma/yrese.git remote_base=f4d0f8f71a62c3e55a57a79ad57092b10620cc70 remote_non_target=4f3185217ec6f2309bab38a8fdbf25112b8f141204b139449686f993039899f2 cas=exact-ff-lease human_gate=required verifier=planned`。Current action=RECOVERY IMPLEMENTATION by root sole maintainer; fresh exactly5 IMPLEMENTATION review pending。
  - s1_recovery_implementation_partial: inline verifier source `2a7598010a26a3f6e303ff6c25db257b902a378bd8909769f1c27d62626e46bd` / 10934 LF lines。signal protected40/post-handoff24/drain1+drain2 exact54/mutation11とV12 supervisorのmanifest/boot/request/authority/raw-Git/capped-PGID/no-replace/claim/result/lock/wait/launcher/AC9P基礎に加え、実sentinel clean-exec intent/challenge/ready/guardian、system `os.posix_spawn` FD8→FD9、bash同一PGID/descendant-lock継承、bash ready→mutation→release、bash exact reap、sentinel-last TERM/group extinction、resume no-signal state machine、durable result O_EXCL/upgrade、future postcommit/preclaim oracle、V12 execution-context fail-closed manifest、GH zero→queued→completed→jobs→deploymentsの実poll core/offline exact8-endpoint guard、live/persisted authority bundle分離、1秒間隔ESRCH二回観測、partial-ready/lock-drift crash resume、base/commit/unknown classification+V2 upgrade、`--classification-resume` terminal経路、actual bash bootstrapを実装した。Production `--recovery-push`はfresh/free monitor lock→再preclaim→atomic claim→pre-spawn再検証→fork-release sentinel→same-PGID bash→pre-release再検証→mutation release→exact child reap/group extinction→fresh classification→COMMIT時AC9P terminalへ接続済み。Actual lifecycle happy pathとexact10 crash boundary、claim orphan/link-nlink2/loser-temp reconciliation、call-site binding guardは同一sourceのoffline実processでPASSし、crash cutでresult0/residue0を確認した。両Python exact recovery self-test、live read-only tool/config/base raw/replace-graft/remote、diff/SSOT173/scripts、tracked exact4 overlay secret scanはPASS。V12 context oracleはauthority固定dev `16777233`に対するlive `16777228`をexact `RECOVERY_CONTEXT_INVALID`でSTOPしrunParentを作成しない。これはpartial implementation evidenceのみ。Production mutation pathは実装済みだが、V13 PLAN/IMPLEMENTATION independent review、context-bound live preclaim/push/AC9P、human approval、commit/pushは未実施であり、V13 PLAN再reviewまでrunParent作成・claim・signal・pushをSTOPする。Current action=V13 PLAN review + recovery IMPLEMENTATION review。
  - s1_recovery_current_validation_preflight: current source `2a759801…` / 10934 LFに対しworkspace typecheck、1829 tests（local PostgreSQL14 expected skip）、lint entrypoint、OpenAPI、calculation purity、boundaries、SSOT173、SBOM232、build（Next.js 12/12）をPASS。`pnpm check:deps`はPostCSS 8.5.16 / GHSA-r28c-9q8g-f849（high=1/critical=0、vulnerable <=8.5.17、patched >=8.5.18）でFAILしたため、current VALIDATIONは未達。WP-4158 exact4はpackage/lockを明示除外するため依存更新を混ぜず、WP-4240へ独立登録する。過去checkpointのhigh=0は当時の事実として保持し、current greenの根拠には使わない。
...
- [ ] WP-4240 remediate PostCSS GHSA-r28c-9q8g-f849 and restore the dependency gate(R2 supply-chain/security)
  - finding: 2026-07-26 current `pnpm check:deps`はPostCSS 8.5.16をhigh severity 1件としてfail-closedに検出した。advisoryはprevious source map auto-loadingのpath traversal / arbitrary `.map` disclosure、vulnerable `<=8.5.17`、patched `>=8.5.18`。現`pnpm-workspace.yaml` overrideは`postcss@<=8.5.11: ^8.5.12`だけで8.5.16を拘束せず、lockにはNext/Vite系から8.5.16が解決されている。
  - acceptance: package manager authorityを不変にし、最小のroot override/lock更新で全PostCSS解決を`>=8.5.18`へ収束させる。unrelated direct/transitive dependency、allowed build script、package runtime/API/UI behavior、CI thresholdを変更せず、lockfileに8.5.16または`<=8.5.17`解決を残さない。advisoryをallowlist/ignoreせず、gateを弱めない。
  - validation: frozen `pnpm install --frozen-lockfile`、`pnpm check:deps`（high=0/critical=0）、PostCSS resolution scan、`pnpm check:sbom`、`pnpm test:scripts`、workspace typecheck/test/build、tracked secret scan、`git diff --check`をPASSする。WP-4158 exact4候補へpackage/lock差分を混ぜず、同sliceが終了または明示的にsupersedeされた後の独立exact package/lock sliceで実装する。
