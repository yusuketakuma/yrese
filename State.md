...
> **ACTIVE SNAPSHOT (2026-09-15, WP-5279 LANDED `ad3aec2` / calculation Oracle review completed — CHANGES_REQUESTED recorded):**
> This block alone is current. Everything below is nonauthoritative.

- **Direction / ownership:** WP-5279はhuman commit認可により`ad3aec2`へ着地済み(15領域監査の低リスク範囲: root boundary、UI ARIA/患者識別子、draft通信のtimeout/abort/再取得、dependency audit failure classification、local Compose PostgreSQL 17系digest pin)。`active_root_writer`は本lane。点数計算packageの監査findingは`Plans.md` WP-5279分離記録へ別WP候補として登録(claimしない)。WP-5278は`6d005ee`、WP-5277は`011ac26`、WP-5276は`a463fac`へ着地済み。FHIR、reception wallClock、薬剤師確認・確定はgate待ちでpark継続。
- **Git boundary:** local `main` = `9c0b357915919b94a1f4fadeca42d1a05403cf91`、current branch = `refactor/wp-5275-failure-proof-describe-failure` / HEAD = record commits on `ad3aec2`(landing)、`origin/main` = `c3a082919f4965915fb11671b12db402a157d990`。push / merge / deployは行わない。
- **Dirty ownership:** tracked差分なし。`.harness-worktrees/`、`artifacts/`、`ui-test-tools/` とsecondary worktreeはuser-owned / protectedで、cleanup、merge、stage、内容参照をしない。
- **Active plan / boundary:** WP-5279は`ad3aec2`へ着地しCURRENT=0・READY=0。tenant/system認可、append-only DB/migration、draftのserver autosave/idempotency、structured observability、PHI/retention、backup/restore/DR、lint/fixture policy、PostgreSQL major version alignmentは別のhuman/security/privacy/data-integrity gateへ分離し、推測実装しない。算定findingはOracle reviewで確定不具合A1/A2/F1/F3/F4-確定分・P2系A3/A4/A5/F2・懸念C1/C2・SSOT送りC3/C4/F5へ確定済み(分離記録参照)。すべて別WP候補でREADY昇格・risk分類・R2+ reviewまで着手しない。
- **Security / privacy / offline:** WP-5279は一時rootのsynthetic sentinelだけで清掃・スキャン境界を検証する。credential、production data、PHI/PII、migration環境適用、DB mutation、external send、外部状態変更は追加しない。Oracle送信packetは計算package+SSOTの非機密12件のみ。
- **Process gate:** 15領域の4件のread-only auditは完了。算定 Oracle review(session `wp5279-calculatio-review`)は実行済み・CHANGES_REQUESTED、transcriptは`~/.oracle/sessions/wp5279-calculatio-review/`に保存。ChatGPT側archiveはskipped(artifact-save-failed)。local oracle patch `pro-deep-fallback-0.20.0`はinstalled 0.21.1とversion不一致でverify失敗 — Pro未利用時のfallback経路は未検証扱い。高リスクfindingの実装承認・専門家レビュー・human gateは未成立。
