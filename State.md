...
- **human 承認・決定待ち(次のアクションに必要):** (1) production/staging 環境のプロビジョニング + 適用承認(WP-7102 runbook の対象環境が未定義)。(2) SEC-009 APPROVED を実装する認証 provider WP の起案・承認(postgres 到達経路)。(3) 資格登録/取消 route WP の起案・承認(SEC-010 §2、000020 の表は read 側のみ実装済み)。
