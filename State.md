...
- **Git boundary:** local `main` = `9c0b3579` から fast-forward merge 済み、`origin/main` へ push 済み(いずれもユーザ指示 2026-09-16)— `origin/main` = `d8844ce`+record commits。merge 済み・patch 等価ブランチはローカル・リモート共に削除済み(ユーザ指示「mainに全て取り込まれているのなら削除して良い」)。deploy は引き続き行わない。
- **Dirty ownership:** tracked差分なし。secondary worktree は 2026-09-16 のユーザ指示で整理済み — `wp-5281-*` 3 worktree の未 commit 実験内容(3 コピー同一)は `audit/wp-5281-all-integration` ブランチの `09a70dc` へ archive してから worktree を除去、`harness/worker/a3643191` worktree は除去(未 merge commit `453d466` は branch 上に保持)、stale `yrese-main` 登録は prune。`artifacts/`、`ui-test-tools/` は user-owned / protected のまま(cleanup、merge、stage、内容参照をしない)。
