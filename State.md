...
> **ACTIVE SNAPSHOT (2026-09-18, IMPLEMENT — WP-7204/WP-7205 着地; migration 000017 ローカル適用済み; CURRENT=0 / READY=0):**
> This block alone is current. Everything below is nonauthoritative.

- **Direction / ownership:** 2026-09-18: ユーザ許可「migrationの適用とプッシュを許可」により origin/main へ push 済み、migration 000001〜000014 をローカル PostgreSQL `yrese_dev`(port 5433、ユーザ `yusuke`)へ適用済み(WP-7102 のローカル分)。続けて **WP-7204 資格確認 snapshot route を `8c778bd` で着地** — POST/GET `/reception/{id}/eligibility-snapshots`、INS-0007〜0010、append-only・冪等再送・Postgres 単一 tx・監査 eligibility.*/insurance.viewed、queue entry `eligibility` 公開(API-006 0.3.2)。さらに **WP-7205 処方箋原本 metadata を着地** — DOM-002 §4.2a(2026-09-17 一括 APPROVE で【HG】解消)、`sourceMetadata` additive schema 追加、migration 000017(8 列 nullable + 全-or-必須 CHECK・valid_until≥issue_date・refill 整合)を `yrese_dev` へ適用済み、legacy 行は sourceMetadata キーなし hash で後方互換受理、Web は原本情報入力欄 + 発行日+4日入力補助 + ClinicalAlert(DOCUMENT_VALIDITY)期限警告。`active_root_writer` は本 lane。`claimable=false`/`POINTS_ONLY_COPAY_BLOCKED` 固定を維持。
- **ループ終端状態(更新):** R1 残り WP-7202/WP-7203 は R3(human pre-review record 必須)。WP-7205 は着地済み(DOM-002 §4.2 の一括 APPROVE で【HG】解消済み)。明示【HG】(production/staging 環境適用・security 等)、【EXT】外部仕様未入手は引き続き fail-closed。**gate なしで実装可能な WP は残っていない。**
- **human 承認・決定待ち(次のアクションに必要):** WP-7105(queue bound/C-021)、WP-7102 の残り環境(migration の production/staging 適用 — ローカルは済み)、WP-7101(認証境界/C-083・security)、WP-7202/7203 の R3 pre-review record、次 WP の queue 昇格(READY/CURRENT)。
