...
> **ACTIVE SNAPSHOT (2026-09-16, PLAN_ONLY — `Plans.md §18` 調剤レセコン機能ギャップ再評価と実装ロードマップを登録; CURRENT=0 / READY=0):**
> This block alone is current. Everything below is nonauthoritative.

- **Direction / ownership:** ユーザ指示 2026-09-16「Plans.md のプランをレビュー、コードベースを探索し、調剤レセコンとして不足する機能と実装までの道筋を計画」により `Plans.md §18`(工程 R0〜R7、WP-7101〜7806 = 41 WP、うち新規 30 / 既登録詳細化 11)を **inventory として登録**(claim ではない。READY slot 非消費)。§1 の stale ブランチ記載を訂正、§1.1 を歴史記録化。**コード・SSOT・migration・gate はいずれも変更していない。** `active_root_writer` は本 lane。`claimable=false`/`POINTS_ONLY_COPAY_BLOCKED` 固定・API/DB/claim 配線なしを維持。WP-5281 は `24319bf`、WP-5280 は `b6d38e6`、WP-5279 は `ad3aec2` へ着地済み。
- **§18 の構造的発見(次の work selection の前提):** (1) postgres mode に認証済み到達経路がない(`dev_headers` は in_memory 限定 → durable mode で全 PHI route 403)、(2) 受付状態遷移 route がない(WAITING 固定)、(3) 患者 write route がない。READY 昇格推奨は WP-7104(North Star 部分 E2E harness、gate なし)→ WP-7103(outbox runner、local/CI sink 限定)。以降の実装 WP は契約 SSOT 起案(API-006 改版、C-028、coverage/eligibility/dispensing 契約、DOM-002 §4.1 拡張)が先行条件。human 決定待ち: queue bound(C-021)、migration 適用(C-058)、認証境界(C-083)。
