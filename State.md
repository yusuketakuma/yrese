...
> **ACTIVE SNAPSHOT (2026-09-18, IMPLEMENT — WP-7203 着地・push 済み(`19e9434`); WP-7101/7301/7303/7304 の SSOT 起案を PROPOSED で作成; CURRENT=0 / READY=0):**
> This block alone is current. Everything below is nonauthoritative.

- **Direction / ownership:** 2026-09-18: WP-7202/WP-7203 pre-review packet(`f2aa1aa`)の全決定(D-1〜D-5)をユーザが一括承認。WP-7202(患者登録・更新 API)を着地・push 済み(`ffd6ed7`)。**WP-7203 保険・公費 Coverage API を着地** — GET/POST `/patients/{id}/coverage`(asOf 必須・append-only・supersede 訂正のみ・Idempotency-Key・`insurance.viewed`/`insurance.updated` 監査)、migration 000016(3 表 + append-only trigger + supersede 一意 + patient scope index)を `yrese_dev` へ適用済み(000017 先行適用のため手動 backfill で整合)。INS-0001〜0006 を shared-kernel へ実装登録、UIX-001 §12.3 に SCR-007-R/C(0.2.2)、Web 患者 coverage パネル(記録値 honest 表示・CAL-R-024 不侵犯)。R3 gate は別 context read-only review 2 ラウンド(MEDIUM3/LOW4 → 修正 → LOW3 → 修正 → FINDINGS_NONE、frozen diff sha256 2e14f411)。`active_root_writer` は本 lane。
- **ループ終端状態(更新):** R1 キューは全件着地済み(WP-7201/7202/7203/7204/7205)。明示【HG】(production/staging 環境適用・security 等)、【EXT】外部仕様未入手は引き続き fail-closed。次は R2 以降(SSOT 起案待ち)または残置 human gate。
- **human 承認・決定待ち(次のアクションに必要):** (1) SSOT 起案 3 件の承認 — **MST-003**(synthetic マスター基盤 → WP-7301/7303)、**PRD-001 0.1.2**(M4 へ前回 Do 追加 → WP-7304)、**SEC-009**(認証 context 注入境界 → WP-7101、security review 必須)。(2) WP-7105 queue bound 決定(C-021、INV-20260730-01 の選択肢 (a)現状維持/(b)防御的 cap)。(3) WP-7102 の残り環境(production/staging 適用)。(4) WP-7302 は DOM-002 §4.2(b) 承認済みだが R3 → 実装前に human pre-review record が必要(かつ WP-7301 前提)。本 SSOT 起案分の push 許可も未指示。
- **§18 の構造的発見(次の work selection の前提):** (1) postgres mode に認証済み到達経路がない(`dev_headers` は in_memory 限定 → durable mode で全 PHI route 403)のみ残存。(2) 受付状態遷移 route なしは WP-7201 で、(3) 患者 write route なしは WP-7202 で解消済み。
- **Git boundary:** `origin/main` = `19e9434`(WP-7203 着地分まで push 済み — ユーザ許可 2026-09-18「残っているタスクを全て実行」)。本 commit は SSOT 起案(MST-003/PRD-001 0.1.2/SEC-009 全て PROPOSED)+ 記録更新のみで local `main` に載り、**push は未指示のため未実施**。merge 済み・patch 等価ブランチはローカル・リモート共に削除済み。local/remote 共に `main` のみ。deploy は引き続き行わない。
