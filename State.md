...
> **ACTIVE SNAPSHOT (2026-09-18, IMPLEMENT — WP-7204 資格確認 snapshot route 着地; migration 000014 ローカル適用+push 済み; CURRENT=0 / READY=0):**
> This block alone is current. Everything below is nonauthoritative.

- **Direction / ownership:** 2026-09-18: ユーザ許可「migrationの適用とプッシュを許可」により origin/main へ push 済み、migration 000001〜000014 をローカル PostgreSQL `yrese_dev`(port 5433、ユーザ `yusuke`)へ適用済み(WP-7102 のローカル分。production/staging 適用は別 gate のまま)。続けて **WP-7204 資格確認 snapshot route を着地** — POST/GET `/reception/{id}/eligibility-snapshots`(POST=insurance:write+reception:read、GET=insurance:read+reception:read、INS-0007〜0010、asOfDate 必須の明示業務日付、append-only・冪等再送 200・別内容 409、Postgres 単一 tx で snapshot+紐づけ+監査、in-memory undo 補償、監査 eligibility.verified/provisional_recorded/insurance.viewed、queue entry `eligibility` 公開 = API-006 0.3.2 additive、Web は受付行バッジ・metrics・launch 画面を snapshot 由来へ)。直前: WP-7201 受付状態遷移 API `f96e236` 着地。`active_root_writer` は本 lane。`claimable=false`/`POINTS_ONLY_COPAY_BLOCKED` 固定を維持。
- **ループ終端状態(更新):** R1 残り WP-7202/WP-7203 は R3(human pre-review record 必須)、WP-7205 は R2(DOM-002 §4.2 承認済み)で queue 昇格待ち。明示【HG】(production/staging 環境適用・security 等)、【EXT】外部仕様未入手は引き続き fail-closed。**gate なしで実装可能な WP は残っていない。**
