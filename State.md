...
> **ACTIVE SNAPSHOT (2026-09-15, WP-5280 LANDED `b6d38e6` — calculation finding remediation; CURRENT=0):**
> This block alone is current. Everything below is nonauthoritative.

- **Direction / ownership:** WP-5280(点数計算 Oracle 確定 finding の是正)はユーザ指示「これらを改善する計画を立ててから実装」により`b6d38e6`へ着地。`active_root_writer`は本lane。`claimable=false`/`POINTS_ONLY_COPAY_BLOCKED`固定・API/DB/claim配線なしを維持。WP-5278は`6d005ee`、WP-5279は`ad3aec2`へ着地済み。FHIR、reception wallClock、薬剤師確認・確定はgate待ちでpark継続。
- **Git boundary:** local `main` = `9c0b357915919b94a1f4fadeca42d1a05403cf91`、current branch = `refactor/wp-5275-failure-proof-describe-failure` / HEAD = `b6d38e6`+record commit、`origin/main` = `c3a082919f4965915fb11671b12db402a157d990`。push / merge / deployは行わない。
- **Dirty ownership:** tracked差分は record 2 path(Plans.md/State.md)のみ。`.harness-worktrees/`、`artifacts/`、`ui-test-tools/` とsecondary worktreeはuser-owned / protectedで、cleanup、merge、stage、内容参照をしない。
- **Active plan / boundary:** WP-5280 で実装したもの = A1 bigint化、A2対象単位排他、A3 factory入力snapshot+money値freeze、A4異常入力拒否、F1/F4排他(非対称`blocksGroupIdPrefixes`機構含む)、F2特別A→確定BLOCKED、F3 effectiveTo=2027-05-31、A5-lite(applicationKey/rejection step)、C1 ITEM_CALCULATED強制、C2置換型排他、F6軽微。deferred・gate送り = C3・C4・F5・A5構造化field(CAL-008改版事項)・F2丸めevidence本体。CURRENT=0・READY=0。
- **Security / privacy / offline:** credential、production data、PHI/PII、external send、外部状態変更は追加しない。Oracle送信packetは計算/money package+非機密SSOTのみ(前回packet準拠)。
- **Process gate:** WP-5280 frozen-diff 独立 review は3段階で完了 — v1(`671c090e…a117f`)CHANGES_REQUESTED(疎配列 shape 検証不備+回帰不足2件)→修正後 v2(`8bff3685…b1e8`)`wp5280-impl-review-v2` PASS→補足反映 v3(`1010e9da…0d10d`)`wp5280-impl-review-v3` PASS。critical finding 0。算定 Oracle review(`wp5279-calculatio-review`)の CHANGES_REQUESTED findings は確定分を閉じた。local oracle patch `pro-deep-fallback-0.20.0`はinstalled 0.21.1とversion不一致でverify失敗のまま — fallback経路は未検証扱い。
- **Validation(WP-5280):** calculation 114 tests PASS、money 17 PASS、workspace全test PASS(API 1,001 + 63 DB-gated skip、Web 758、trace 61、audit 206、contracts 136等)、workspace typecheck、`check:calculation-purity`、`check:boundaries`、`check:ssot-index`(185)、`check:deps`(high=0/critical=0)、`check:sbom`(249)、`test:scripts`、`check:openapi`、`lint`、`git diff --check` PASS。**`check:secrets`はclean HEAD(`7ef6f43`)でも tracked symlink `CLAUDE.md`→`AGENTS.md` が protected-scope error を起こし失敗 — 本 diff 起因ではない既存不具合として記録(gate script 側の修正は別WP候補)。**
- **Seven-day gates:** FHIR判断packet、人間承認、migration環境適用、production/deploy、HPKI legal authority、REG-004 RB-003、薬剤師確認・確定は未解消。tenant/system認可、DB/migration DDL、draft idempotency/durable recovery、observability sink、PHI retention、backup/restore/DRも未解消。WP-5280はfrozen-diff review PASSを経て着地済み。
