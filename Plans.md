...
- [ ] WP-4050 reception audit event persistence boundary(codex 提案 SELF-SCAN-20260709-29、SEC-007高リスク隣接)
  ...
  - 目的: 受付作成・将来の受付取消を、監査イベント台帳だけでなく API 実行時の audit sink へ接続するための境界を定義する。SEC-007/WP-2009 のハッシュチェーン本実装前でも、偽の tamper-evidence を出さない in-memory/test adapter と本番BLOCKERの切り分けを fable5/opus4.8 が裁定する。
...
- [ ] WP-4051 reception idempotency durability boundary(codex 提案 SELF-SCAN-20260709-30、DB計画との接続待ち)
  - 発見根拠: WP-3009-BE/93aefa1 の `InMemoryReceptionRepository` は `(tenantId, pharmacyId, idempotencyKey)` 境界で200再送/409 conflictを正しく固定しているが、永続DB・transaction・unique constraint は未導入。プロセス再起動や複数APIインスタンスでは idempotency record が共有されず、受付二重登録防止の実運用品質が成立しない。
  - 目的: 受付キューの本番永続化に入る前に、DB schema / unique constraint / transaction boundary / repository interface の要件を整理する。Claude側が作業中の `docs/plan/database_construction_plan.md` と衝突しないよう、本WPでは計画候補登録に留め、DB計画統合後に正式WP化する。
...
- [x] WP-4074 patient search cursor authenticity/privacy hardening(codex 提案 SELF-SCAN-20260710-19、HIGH、fable5 PLAN_APPROVED、本WPで実装)
  ...
  - key境界: pure config resolverは `YRESE_PATIENT_SEARCH_CURSOR_HMAC_KEY` の exact unpadded base64url 32 bytesだけをconfigured keyとして返す。blank/malformed/noncanonicalは値を含まない固定error、PostgreSQL/production/staging/unknown modeは欠落時startup fail-closed。exact development/test + explicit in-memoryだけ explicit ephemeral decisionを返し、乱数生成はmain composition rootまたはnon-postgres buildServer test seamに限定。`BuildServerOptions` はraw keyでなくcodecを注入し、PostgreSQL modeのcodec欠落を構築前拒否する。v1はsingle active keyでrotation時の旧cursor失効を許容し、TTL + key id/keyring overlapは別SSOT/WPのまま。
  - テスト・検証: same-key codec/別server間の正常page、different key/query/tenant/pharmacy、length-prefix境界、tokenにquery/queryHash/tenant/pharmacy/unsalted SHA-256候補がないこと、offset/MAC/version/extra-key/key順/whitespace/inner tail bits/outer padding/UTF-8/oversize/legacy、raw JSON `-0`/exponent/string offset、safe-int境界を固定。route forged/legacy 400、config mode matrix、raw key非露出を検証。API 84 PASS + PostgreSQL integration 5 expected SKIP、全workspace typecheck/test/build、OpenAPI drift、boundaries、secrets、deps、SBOM、script harness、diff-checkはPASS。read-only Opus最終reviewは `APPROVED`、blockerなし・commit認可。

- [ ] WP-4075 reception patient identity single-source enforcement(codex 提案 SELF-SCAN-20260710-20、HIGH medical/data-integrity、fable5 PLAN_APPROVAL待ち)
  ...
  - 提案scope: scoped `findById` 後に `patient.patientId === requested patientId` をfail-closed検証し、不一致時はreception repositoryを呼ばず固定PHI-free errorへ収束する。`ReceptionCreateInput` から冗長 `patientId` を削除し、両repositoryの永続ID/idempotency比較/responseを検証済み `patient.patientId` だけから導出する。contract/OpenAPI/DB migration/schemaは変更しない。
  - テスト案: malicious patient lookupが別ID snapshotを返すとreception create 0 call + 固定error、in-memoryでdual identityが型/入力上表現不能、PostgreSQL INSERT parameterとreturned snapshotが同一patientId、既存valid/idempotency/tenant-pharmacy scopeを維持。medical safety/data integrity/privacy reviewを通すまで実装しない。

- [ ] WP-7001 Phase 1 DynamoDB persistence foundation + first aggregate synthetic proof(fable5 PLAN_APPROVED、HIGH、M1完了・後続HOLD)
