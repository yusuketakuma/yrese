...
- [ ] WP-4074 patient search cursor authenticity/privacy hardening(codex 提案 SELF-SCAN-20260710-19、HIGH、fable5 PLAN_APPROVAL待ち)
  ...
  - 提案scope: `apps/api` server-only の injected `PatientSearchCursorCodec` を追加し、tokenは version + safe nonnegative offset + HMAC-SHA256だけを保持する。MAC入力にdomain tag、version、trusted tenant/pharmacy、trim済みquery、decimal offsetを長さ曖昧性のないcanonical formで含め、exact keys・canonical base64url・constant-time比較・generic invalid resultを強制する。legacy unsigned cursorはdowngrade decodeせず `400 PAT-0001`。contract/OpenAPI/DB migration/package/lockは変更しない。
  - key境界: `YRESE_PATIENT_SEARCH_CURSOR_HMAC_KEY` はexact unpadded base64url 32 bytes。blank/malformedは固定error、PostgreSQL/production/staging/unknown modeは欠落時startup fail-closed。exact development/test + explicit in-memoryだけ process-local random keyを許容し、値はlogしない。v1はsingle active keyで、rotation時の旧cursor失効を許容。TTL + key id/keyring overlapは別SSOT/WPとする。
  - テスト案: cross-instance same-keyの正常page、tokenにquery/queryHash/tenant/pharmacyが現れずcandidate照合不能、offset/MAC/version/extra-key/非canonical/oversize/legacy改変、different key/query/tenant/pharmacy、safe-int境界を拒否。routeのno-store/正常page2/forged 400、config mode matrix、raw key非露出を固定する。fable5承認までは実装・config変更なし。
...
- [ ] WP-7001 Phase 1 DynamoDB persistence foundation + first aggregate synthetic proof(fable5 PLAN_APPROVED、HIGH、M1完了・後続HOLD)
  ...
  - M1検証: audit 88/88、全workspace typecheck/test、audit build、boundaries、secrets、deps、SBOM、script harness、full build、diff-checkがPASS。local PostgreSQL integration 5件は `TEST_DATABASE_URL` 不在でexpected skip、DB操作なし。AWS SDK/package/lock、DynamoDB Local harness、adapter/persistence writeは未変更。後続laneは別途fable5計画承認までHOLDを維持する。
