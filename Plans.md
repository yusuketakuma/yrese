...
- [x] WP-5001 DB設計 SSOT パック(DB-001〜004、PROPOSED)— DB-0。DB-001: スキーマ設計規約(tenant/pharmacy必須・money/date型・enum二重実装禁止)。DB-002: 前方一方向・3段適用・明示運用操作のマイグレーション規律。DB-003: テナント分離DDL/Repository方針(RLSはBLOCKED_SECURITY_REVIEWまで候補)。DB-004: 保存期間・削除方針(年限未確定は削除しない、BLOCKED_LEGAL_REVIEW維持)。
- [ ] WP-5002 開発環境 PostgreSQL + マイグレーション基盤(codex。Repository interface の DB 実装を in-memory と差し替え可能に)— DB-1(WP-5001 APPROVED が発行条件)
- [ ] WP-5003 患者・受付リポジトリの DB 実装差し替え(API-001/006 の「DB化時」注記の実行。DOM-002 系 data_model SSOT APPROVED が前提)— DB-2
- [ ] WP-5004 監査ログ永続化 + ハッシュチェーン(WP-2009 と統合。SEC-007/008 論理層規律の実装)— DB-3
