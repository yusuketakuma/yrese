...
- [ ] WP-4140 make OpenAPI drift artifact target fail closed(MEDIUM api-contract/tooling) — DISCOVERED
  ...
  - proposed_scope: checker、script harness、3台帳のexact5。generator/renderer/contracts/artifact/docsは不変候補。
  - required_design: real regular non-symlink final targetをrace-safe same-handle no-follow readで検証し、scope/I/O failureを固定非echoにする。regular match/drift semanticsを維持する。
  - status_reason: WP-4139とmutation/read-only責務を分離。WP-4139 landing後にfresh mapping/pre-planする。
