...
- [ ] WP-4072 SQL secret scan coverage(codex 提案 SELF-SCAN-20260710-17、fable5 PLAN_APPROVED、HIGH、未実装)
  ...
  - 修正方針: `scripts/check-secrets.mjs` の既存 text extension allow-list に `.sql` だけを追加し、secret pattern、directory/file ignore、per-line `secret-scan: allow`、path/line/type のみを出す value-redacted error contract は変更しない。対象を binary/生成物や未関係拡張子へ広げない最小修正とする。
  - テスト・検証: `scripts/check-scripts.mjs` に clean SQL の pass、synthetic secret を含む SQL の exit 1、SQL 内の明示 allow marker の pass、finding が path/line/pattern type を示す一方で secret value 自体を出力しない redaction assertion を追加する。`pnpm test:scripts`、synthetic SQL false-pass reproduction、`pnpm check:secrets`、`pnpm check:boundaries`、`git diff --check` を実行する。

- [ ] WP-4073 CI PostgreSQL integration fail-open closure(codex 提案 SELF-SCAN-20260710-18、fable5 PLAN_APPROVED、HIGH、未実装)
  ...
  - 承認済み方針: CI test runtime は fable5 裁定の interim PostgreSQL 16 とし、official Docker Hub `postgres:16` OCI index digest `sha256:be01cf82fc7dbba824acf0a82e150b4b360f3ff93c6631d7844af431e841a95c` を implementation pin candidate とする(2026-07-10 live verification)。version+digest pinの disposable service、CI専用database/user、health check 後に `TEST_DATABASE_URL` を設定してintegration 5件を実行する。
  - 境界: CIでは `CI=true` かつ `TEST_DATABASE_URL` 欠落を skip でなく fail-closed、local no-env の明示skipは維持する。fixtureは合成データのみ、schema単位の作成/破棄を継続し、外部/dev/prod DBへ接続しない。production Aurora の target major は別途 SSOT_UPDATE 対象であり、interim CI PostgreSQL 16 から推測・流用しない。本WPは未実装のため workflow/package/lock/migration は変更せず、DB操作も行わない。
  - 想定検証: CI integration 5 PASS、missing-env negative gate、pinned service health、既存API/unit/typecheck/build/boundaries/secrets/SBOM、`git diff --check`。本WPでは計画登録のみで未実装。
...
- [ ] WP-7001 Phase 1 DynamoDB persistence foundation + first aggregate synthetic proof(fable5 PLAN_APPROVED、HIGH、実装HOLD)
  ...
  - 状態: fable5 `PLAN_APPROVED` 受領済み。(a) WP-4066 landing は充足済み。実装着手は残る (b) fable5 から DB-005 §6/§10 への A/B/C・port signature・同一PK pin 反映通知を受領した後。それまでは実装 HOLDで、AWS SDK/package/harness/adapter のコード変更なし。DB-005 pin の landing は未確認。
