...
- [~] WP-4157 map FHIR validator / IG Publisher / SUSHI compatibility candidates(LOW read-only toolchain research) — LOCAL_LANDED / REVIEW_INFRA_BLOCKED / INDEPENDENT_VERIFY_REQUIRED
  ...
  - acceptance/review: 2026-07-18 independent verificationはJP Core tag `1.2.0`をannotated tagからcommit `c06f02059c2a8aed6a33d624c9eee6fe0669ef06`へ解決し、historical `publisher.jar` 2.0.17のofficial release asset id/name/208,967,243 bytes/SHA-256 `878c78531058961fdf101a462af6657bfb79692a91c29623afac108963ae233d`を再照合した。current candidatesはofficial GitHub release APIでPublisher2.2.11 `publisher.jar` 230,671,837 bytes/SHA-256 `a981af86bca3f3a22ee15b9d4ee3c97d63219b7a14e49d0a525e10bbfc71a911`、validator6.9.12 `validator_cli.jar` 187,081,756 bytes/SHA-256 `0e53ab1d1a6f1e35f505255c0b8ce10a35fcf27e6e96b503640f784cd07e5ad6`を確認し、npm official metadataで`fsh-sushi#3.20.0` Apache-2.0/SRI `sha512-fW5H+XANg75WoU2eikmDx62Cf8ow6whxy+3RX7SoRES4HxnCeQ6MMq3BlS5VLrKM010l6Tj8fmuJ0nwPETtC+Q==`を再現した。Publisher/validator source POMはJava17 target、SUSHI v3.20.0 READMEはNode22推奨・18/20 support・22超はofficially unsupportedを明示し、yrese local/CI Node24とlocal Java不在はcompatibility proofでない。historical workflowの`check-latest` Nodeとfloating global SUSHIはexact reproduction不能のまま維持する。binaryを将来取得する場合はGitHub release APIでasset id/name/size/digestを先に固定し、HTTPSで`github.com`から`release-assets.githubusercontent.com`へのexactly one redirectだけを許可、256MiB上限/connect15s/total120s/temp→Trash/raw SHA照合を必須とし、signed redirect queryをlog/evidenceへ残さない。本WPはbinary download/install/cache reuseを行わない。historical characterization、isolated cacheのlocked-package validator、Java17/Node22 minimal IG build、offline/cache-onlyとterminology-networkを別laneとして、`latest`/floating npm/ambient cacheをlock evidenceにせず、tools conflict/missing canonical、clean resolution、FHIR/supply-chain/legal reviewとWP-0053a/b human gate前にversion selection/lock/CI/runtime/SSOT/toolchain adoption/license grant/WP-0053b unblockを行わない。exact4の`git diff --check`、SSOT index173、script regression、path/index、archive residue0とtracked HEAD+exact4 overlay cwd fail-fast secret scanだけをPASSとし、live `pnpm check:secrets`は既存user-owned `.codegraph` symlinkのprotected-scope exit1でfail-closed/non-greenとして分離する。COMMIT_GATE5/5を満たした場合だけexact-stage/commitし、本条件文を含む最初のtracked-clean local HEADをfinalization targetとする。このcheckboxと完了宣言はpost-commit independent review、remote/PR ancestry確認、conditional fast-forward push、local/origin/PR parity、origin/main `27d6144`不変、deployments0、exact-head CI terminal success、PUSH_GATE5/5の全成立時だけ有効で、成立前はFINALIZATION_PENDING、違反時は無効として修正gateへ戻る。成功後は追加ledger commit不要。
  - release_asset_ids: Publisher2.2.11 `publisher.jar`=`478216867`; validator6.9.12 `validator_cli.jar`=`478124375`。future binary retrievalはacceptance/review記載のasset id/name/size/digest pinとredirect境界を満たす。
  - rollback: safe rollbackは今回のexact4 finalization target commitだけをrevertする。historical WP-4157 evidence `42fa277`/`ffa1fd0`を直接revertせず、WP-4158〜4160、WP-0053bと関連EVIDENCE/State参照のreverse-dependency impact、conflict、downstream invalidation、再validationをfresh mappingする別reconciliationまでSTOPする。runtime/data/artifact rollback不要。
  - landing_record: implementation `42fa277`とlanding ledger `ffa1fd0`はsafe feature branchに含まれ、当該checkpoint時点ではindependent verification pendingだった。current fresh closureの成否は上記EXTERNAL_STATE_CONDITIONALに従う。
