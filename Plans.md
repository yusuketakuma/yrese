...
- [~] WP-0003 二系統運用・エージェント統率SSOT(docs/agents/ 15文書)— フォーク実行中
...
- [ ] WP-1001 monorepo scaffold(pnpm workspaces / TypeScript / vitest / lint / turbo)+ CI設計メモ
- [ ] WP-1002 packages/shared-kernel: branded ID型(TenantId/PharmacyId/PatientId/PrescriptionId/ClaimId/EventId等)、システムモード(NORMAL〜RECOVERY_SYNC)、PENDING系status、BLOCKER種別、error/warning code registry型、permission scope型 + unit tests
- [ ] WP-1003 packages/money: Decimal ベース金額・点数型(floating point禁止)、丸め境界インターフェース(丸め実装は公式根拠確認まで placeholder + BLOCKED表明)+ tests
- [ ] WP-1004 packages/date-time: 処方日・調剤日・受付日・請求月の明示型、JST/タイムゾーンポリシー + tests
...
- [ ] WP-2001 apps/api scaffold(フレームワーク選定メモ + 最小起動 + healthcheck)
...
- [ ] WP-3001 apps/web scaffold(Next.js + 医療UI原則に沿ったshell、グローバル状態表示領域)
