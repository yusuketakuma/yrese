...
- [ ] WP-3009 SCR-001 受付ダッシュボード実体化(受付キュー契約 SSOT 起草含む)— UI-2。API-006 v0.2.0 は APPROVED(30f09a3)、WP-3009-BE で shared-kernel/contracts/apps/api の受付キュー backend を実装。
...
- [ ] WP-5001 DB設計 SSOT パック(スキーマ設計規約 / マイグレーション規律 / テナント分離 DDL 方針 / 保存期間・削除方針[年限確定は BLOCKED_LEGAL_REVIEW])— DB-0
