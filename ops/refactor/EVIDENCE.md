...
|---|---|---|---|
| WP-4232 | uncommitted exact7 | `Plans.md`; `State.md`; `ops/refactor/STATE.md` | `COMMIT_READY`: fixed-JST(+09:00) acceptedAt projection delegates shifted UTC parts to canonical CalendarDate across InMemory/PostgreSQL/API/Web; Web default date, queue membership and accepted-time presentation share one apps-layer fixed-JST snapshot. PLAN/IMPLEMENTATION/BUG_REFACTOR/VALIDATION gates passed 5/5 fresh. Historical-IANA display authority, rollback/evidence, and workspace-count P2s were remediated; all re-reviews PASS. Current diff focused API497 + Web136, workspace1818 + 14 expected PostgreSQL skips because `TEST_DATABASE_URL` is absent (not real-DB zero-skip proof), typecheck/test/build and standard non-secret gates PASS; live secrets fail closed on existing workspace scope, tracked HEAD+exact11 clean overlay PASS. Root-observed ephemeral browser session restored `?date=0001-01-01` exactly with alert 0, but no screenshot/trace/console artifact was saved. Rollback reverts the implementation commit plus ledger-only finalization commit and reruns focused API497/Web136 and standard gates; no DB/schema/data rollback, but historical IANA/host date authority risk reopens. No commit/push/real-DB/remote/prod claim. |
