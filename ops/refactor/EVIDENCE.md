...
|---|---|---|---|
| WP-4227 | `COMMIT_PENDING` | `Plans.md`; `State.md`; `ops/refactor/STATE.md` | `VALIDATED / COMMIT_PENDING`: InMemory/PostgreSQL patient lookup/search commands are detached once before scan/query/await with shared own-data primitive scope/query/cursor guards; reception scope helper extraction is behavior-equivalent. PLAN/IMPLEMENTATION/BUG_REFACTOR/VALIDATION gates passed 5/5 after remediation of a P2 non-round-trippable upper-bound next cursor. patient56 + reception166 = focused222, API758 + local PostgreSQL14 expected skips, Web454, workspace test/typecheck/build and standard non-secret gates PASS; live secrets fail closed on existing workspace scope, tracked HEAD+exact7 clean overlay PASS. exact11 commit gate pending; no real-DB/remote/prod/UI/browser/push claim, WP-4050/WP-4151c unchanged and `.omo/` excluded. |
