...
change_log:
  - "0.2.10 (2026-09-19): WP-7404 packet(決定済み、direct user instruction 残タスク一括許可)に基づき `dispensing.recorded` / `dispensing.confirm.denied` を登録し、既存 `dispensing.confirmed` の payload 規律を明記。deny 系は SEC-010 §4 の outcome=denied 規律。実装・production action は含まない"
...
**実装状態: `@yrese/audit` に実装済み(WP-2003/73ffd90, WP-2010/4cf702f)。**
apps/apiはpatient/reception/prescription draftのbounded operationをin-memory/PostgreSQL
audit repositoryへ配線済みである。レジストリ全種別の業務配線やrelease readinessは主張しない。
...
| dispensing.recorded | 調剤実施記録の作成(status未確定) | ○ | payloadはdispensing ID+prescription ID+actorIdのみ。調剤内容・患者識別子はpayloadへ入れない。WP-7404 |
| dispensing.confirmed | 薬剤師確認(調剤記録→DISPENSING_RECORDED) | ○ | `dispensing_records` status遷移と同一tx、outbox `dispense.confirmed` 同時記録。payloadはdispensing ID+prescription ID+actorId。actorは薬剤師(人間責任の明示)。WP-7404 |
| dispensing.confirm.denied | 調剤確認操作の拒否(非資格・scope不足) | ○ | outcome=denied(SEC-010 §4)。理由内訳・免許情報・調剤内容はpayloadへ入れない。targetRefはdispensing ID+actorId。WP-7404 |
