...
depends_on:
  ...
  - CLM-001 claim_scope_matrix(工程7 請求月締め・工程8 請求データロック・工程10 返戻・再請求)
...
集計規律**を確定し、分母分子の業務的な確定は請求 SSOT(CLM 系。CLM-001 工程7・8・10)の
APPROVED 範囲に従う — 請求・返戻の業務定義を本書が先回りして確定しない。
...
- **分母**: 対象請求月に**請求確定**したレセプト件数。「請求確定」の業務定義は
  CLM 系 SSOT の APPROVED 定義に従う(CLM-001 工程7 請求月締め・工程8 請求データロック)。
- **分子**: 分母のレセプトのうち、審査支払機関から**返戻**として返送された件数。
  「返戻」の定義(査定との区別を含む)も CLM 系 SSOT に従う(CLM-001 工程10 返戻・再請求は
  BLOCKED_REGULATORY_REVIEW — 解除前に本KPIの分子を実装しない)。返戻と査定は別KPIとし、
  混合した単一指標を作らない。
...
- 返戻業務フロー(受領・要因分類・再請求)の実装仕様 — CLM 系 SSOT の範囲
- 具体的な公開頻度・閾値・媒体 — QUA-008 の決定手順に従い改版で確定
- 査定率・その他KPIの定義 — 必要になった時点で別 SSOT として起案

## 変更履歴

- 0.1.1 (2026-07-09): opus4.8 レビュー反映(請求確定・返戻の業務定義委譲先を RCP 系[領収証系]から CLM 系[請求系、CLM-001 工程7/8/10]へ訂正・depends_on に ARC-006/ARC-007 を追加)。
