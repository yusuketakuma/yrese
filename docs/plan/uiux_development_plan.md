...
effective_to: null
source_refs:
  - 構築プロンプト v0.2.0 §16(法令適合性・医療安全・UI/UX)
...
- **WP-3007 SCR-013 横断警告・エラー・BLOCKER 表示(U4)**
  ...
  - **medical_safety_reviewerと必要なhuman authorityのレビュー必須**(横断 U4)

### Phase UI-2: 受付・患者線(依存: API-001 済み — UI-1 完了後)

- **WP-3008 SCR-002 患者検索強化(U4)**: 同姓同名警告・類似候補の区別表示(カナ・生年月日の並置強調)。WP-4037(stale response/race guard)・WP-4038(dev header 本番境界)を統合。medical_safety_reviewerと必要なhuman authorityのレビュー必須
- **WP-3009 SCR-001 受付ダッシュボード実体化(U2)**: 受付キュー表示。**前提: 受付キュー API 契約 SSOT の起案→codex 実装可能性レビュー→APPROVED**(API-001 と同じ手順)。契約起草は本 WP に含む
- **WP-3010 SCR-026 LOCAL_ONLY モード UX(U4)**: モード別の禁止操作一覧+理由表示。shared-kernel 判定関数(canConfirmExternal 等)を唯一の判定源とし、UI 側で独自判定しない。medical_safety_reviewerと必要なhuman authorityのレビュー必須
...
- **WP-3014 処方・調剤 API 契約 SSOT パック起草**: AGT-018に従うread-only mapper / pre-plan reviewer / sole Codex maintainer / independent verifierとrelevant domain/human gateを経てAPPROVED後に:
- **WP-3015 SCR-004 処方入力(U4)** / **WP-3016 SCR-010 調剤入力+SCR-014 薬剤師確認(U4)**: 仮/確定・薬剤師確認前後の状態区別を UIX-001 準拠で表示。いずれもmedical_safety_reviewerと必要なhuman authorityのレビュー必須
...
- **WP-3020 SCR-019 請求前点検(U3)** / **WP-3021 SCR-020 月次締め(U4)**: 月次締めは NORMAL モードのみ活性(allowsClaimFinalization)+締め状態の不可逆性を明示。U4はmedical_safety_reviewer、privacy_compliance_reviewer、必要なhuman authorityのレビュー必須
- SCR-021 レセプト出力・SCR-022 返戻は evidence/CLM 実装解除後に採番

### 保留(外部依存解除待ち)

SCR-005(JAHIS 2D)/ SCR-006・008(ONS = WP-0016 人間作業)/ SCR-009(PMH)/ SCR-029 管理(認証基盤の本実装後)

### 廃止

SCR-028 監査ログ閲覧はWP-4254で一般業務Webからsource削除・local検証済み(landing未要求)。監査イベント生成、`audit-log:read`、権限制御API、append-only保存、hash-chain検証はSEC-007の非UI境界として維持し、廃止IDを別画面へ再利用しない。
...
- 0.2.0 (2026-07-29): direct user instructionによりSCR-028をactive UI計画から除外。監査証跡の生成・保全・権限制御APIはSEC-007で維持。
