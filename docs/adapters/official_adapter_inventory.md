...
Official Adapter は、公式仕様・JAHIS仕様・許諾済NSIPS等に準拠してデータを変換・入出力する境界である(v0.2.0 §12)。
...
| 公式仕様 | 電子処方箋管理サービス技術解説書(v0.2.0 指定: 令和8年7月 2.04版以降【要確認】)、記録条件仕様【要確認】、セルフチェックリスト【要確認】 |
| 入出力形式 | 【要確認】 |
...
| 責務外 | **オンライン請求の直接送信・画面自動操作・非公式API送信(実装禁止 — v0.2.0 §19)** |
