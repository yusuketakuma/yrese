...
| R-AUDIT | 監査証跡のproduction運用経路(SCR-028 viewerはretired) | **2026-07-29 current-fact correction**: 2026-07-11に実装されたSCR-028 Web viewerはWP-4254でsource削除・local検証済みで、chain破断のCRITICAL画面表示を安全・compliance mitigationとして扱わない。維持する現行証拠は、contractsのIDs-only投影、repository上の`GET /audit/events` development/test contract(exact tenant/pharmacy、audit-log:read、no-store、全保存chain検証、閲覧ごとのaudit.viewed追記)、reception.created producer、append-only trigger、Postgres repository、runtime配線である。**未実装/未証明**: production tenant/auth・role mapping・incident-response運用、tenant-wide/support/cross-tenant境界、runtime DB privilege分離、物理WORM、保持期間全体の監査済みexport、bounded pagination/verification。APIは応答最大200件でも全chainを読出し、成功readごとに自己増加する | 本番調査経路・見読性・availabilityの証拠不足(G1/G6) | audit event production、IDs-only contract、append-only trigger、hash-chain検証を維持。一般業務Web viewerは緩和策に数えない | separate APPROVED authorization/operations contract → production auth/tenant配線 → retention-complete export/bounded verification → security/privacy/medical/data-integrity review | security/operations | security+privacy+clinical+data integrity | P1(release blocker) |
