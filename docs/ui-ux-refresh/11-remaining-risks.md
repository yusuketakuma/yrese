...
| R-AUDIT | 監査ログ閲覧(SCR-028) | ✅ **実装済(2026-07-11)**: contracts(表示投影・ID のみ・PHI 氏名非含有)+ API `GET /audit/events`(audit-log:read、no-store、**全保存イベントの hash chain 検証**、閲覧自体を audit.viewed で監査、reception 作成が reception.created を発行)+ web 管理画面ビュー(**chain 破断を CRITICAL alert で明示** — 正常に見せない、who/when/what 表、空/エラー/権限状態)。test: api+7 / contracts+9 / web+5。**残**: 永続層(DynamoDB — key codec は既存)への置換・実認証 actor の接続 | 証跡不足(G1 必須) | InMemory 実装+改ざん検知表示 実装済 | 永続層配線 + セキュリティレビュー | admin | security+clinical | P1(永続層 残) |
...
| R-PATCTX | 患者文脈(PatientHeader)全画面横断固定 | ✅ **横断化 実装済(2026-07-11)**: `PatientContextProvider`(RootLayout 直下・遷移後も状態維持)+ `PatientContextBar`(全業務画面で選択中患者を固定表示)+ `useOptionalPatientContext`。患者検索の選択を横断文脈へ委譲、別患者選択で前文脈破棄(H-02)、年齢 computeAgeYears(JST)。test(bar view/変換/未選択)。**残**: 遷移先での再取得は将来 get-by-id API で強化 | 患者取り違え(H-01/H-02) | 全画面横断固定 実装済 | get-by-id API で遷移先の鮮度強化(任意) | frontend | clinical | 済(API 強化は任意) |
...
| R-URLSTATE | 業務日付が URL 非共有 | ✅ **解決(2026-07-11、日付のみ)** | — | 受付日付を `?date=` に反映/復元(history API・非PHI・実在日付検証)。**患者検索クエリ(氏名=PHI)は意図的に URL 非永続**(履歴/リファラ/ログ漏えい防止)。test 114 pass | frontend | — | 済(cursor 等は残) |
| R-RESPONSIVE | テーブルのモバイル/タブレット | ◐ **一部対応(2026-07-11)** | 現場端末で切れ | `.table-scroll` 横スクロールで薬剤名/患者識別を切らない | 列優先/カード化は残 | frontend | a11y | P2 |
...
| R-RESPONSIVE | テーブルのモバイル/タブレット方針未実装 | 未対応 | 現場端末で薬剤名/患者識別が切れる(L-02) | デスクトップは可読 | 列優先/カード化/横スクロール方針を Foundations 化 | frontend | a11y | P2 |
