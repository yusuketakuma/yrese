...
depends_on: [ARC-001, ARC-002, ARC-011, OPS-009(SLA/SLO), packages/shared-kernel(system-mode)]
impacts: [WP-0044 event-sourcing pack, WP-2203 Integration Hub, apps/api, apps/web]
open_questions:
...
可用性の具体数値(SLA/SLO)は本書では定めない(OPS-009 に委譲。数値確定まで BLOCKED_PERFORMANCE_SLO)。
...
- 可用性数値の対外約束 → OPS-009 確定まで BLOCKED_PERFORMANCE_SLO
