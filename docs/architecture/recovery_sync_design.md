...
- 不一致は自動補正せず `CONFLICT_REQUIRES_HUMAN_REVIEW`(@yrese/shared-kernel 実装済み)とする(v0.1.7 §16)。
- RECOVERY_SYNC 完了(未解決タスク 0 + 承認完了)まで、請求前点検・月次締め・レセプト確定は解禁しない(offline_mode_matrix #18-21)。

## 2. 工程分解(v0.1.7 §16 の必須項目を工程化)
...
| R6-2 同期結果レポート | 処理件数・未同期・失敗・要確認の一覧を生成(未解決タスクが一目で分かる — v0.1.7 §8.4) |
