...
approved_by: ユーザー投稿の正式仕様(v0.1.7 / v0.1.8)を原文収録、v0.2.0はdocs/spec/construction_prompt_v0.2.0.mdへ追加保存
```

## 優先規定

本文書には構築プロンプトの各バージョンを収録する。**バージョン番号が新しいものほど優先する**(現行: v0.2.0 > v0.1.9 > v0.1.8 > v0.1.7)。新旧で矛盾する場合は新バージョンの記述が正となる。ただし高リスク領域の停止条件は緩和方向に解釈してはならない(v0.1.6以降の全停止条件は累積的に有効)。v0.2.0以降の大きな追加本文は `docs/spec/construction_prompt_v0.2.0.md` のような版別ファイルにも保存し、Plans.mdから参照する。
...
| v0.1.9 | 2026-07-09 | Open Rececon Platform / NSIPS依存卒業 / Integration Hub / Partner Sandbox / Contract Test Kit / 24時間稼働品質 / データ主権 |
| v0.2.0 | 2026-07-09 | yrese戦略ドクトリン / NSIPS境界隔離 / FHIRネイティブCanonical Core / 品質公開KPI / ルールデータ+純粋関数+イベント再投影 / API-first dogfooding / PH-OSリファレンス連携 / WORM監査・tenant isolation |
