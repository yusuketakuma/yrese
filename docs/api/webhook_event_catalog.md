...
|---|---|---|---|---|
| `reception.created` | WP-4050 command | reception_id, patient_ref(opaque), business_date, version | none | WP-6003 |
...
| `eligibility.snapshot.recorded` | WP-6303 | snapshot_id, patient_ref, verified_method, version | none | Track D |

payload は識別子・版・日時のみを既定とし、本文は scope 付き read API で取得する。患者氏名・生年月日・薬剤名などを event に載せない。

## 2. 配送

- 配送 worker は `outbox_events` の `pending` 行を aggregate 単位の順序で取り出し、partner の DeliveryEndpoint へ POST する。at-least-once。
- 署名: HMAC-SHA256(鍵は endpoint ごと、rotation 時は新旧 2 鍵併記期間を持つ)。header に event_id、timestamp、signature。受信側は timestamp ± replay window 外を拒否する。
...
- DLQ からの再送は tenant 管理者の明示操作とし、自動再送しない。
- 配送済み判定は `delivered_at` の単一遷移(migrations/000005 のトリガ)だけで表現する。

## 3. 購読

- partner は `events:subscribe` scope で event 種別ごとに購読する。購読は PartnerGrant と同じ tenant/pharmacy 境界に閉じる。
