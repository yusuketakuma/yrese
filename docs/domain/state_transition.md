...
- 状態名は shared-kernel の実装済み定数(SYSTEM_MODES / PROVISIONAL_STATUSES / BLOCKER_TYPES)と本書のみを正とする。ローカル定義禁止。
...
| 月次締め済み → 請求ロック | 締め操作者と別の承認(claim:finalize scope)【要確認: 単独薬局運用での二者承認の現実性】 |
