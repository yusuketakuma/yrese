...
- 「JAHISコード」という単一コード体系は存在しない(v0.1.7 §22)。JAHIS 各仕様が参照するコード表(用法コード、医薬品コード指定等)は**仕様ごとに** MST-002 CodeMappingRegistry の code_system として登録する。
...
- 2Dシンボル取込では、コード解決に失敗した項目を**手入力補正キューへ**回し、自動補完しない(v0.1.7 §26)
