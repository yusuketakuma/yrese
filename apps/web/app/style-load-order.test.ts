...
const duplicateRefinementUrl = new URL("./operator-first-refinement.css", import.meta.url);

describe("operator stylesheet load order", () => {
  it("keeps one explicit cascade and loads adversarial corrections last", () => {
    ...
    const navigationIndex = layoutSource.indexOf('import "./operator-first-navigation.css";');
    ...
    expect(layoutSource).not.toContain('import "./operator-first-refinement.css";');
